    ...
    match r.rows[0].data.get("bbox") {
        Some(Value::Object(obj)) => {
            if let Some(Value::Float(min_lat)) = obj.get("min_lat") {
                assert!(
                    (*min_lat - 0.0).abs() < 1e-10,
                    "min_lat should be 0.0, got {}",
                    min_lat
                );
            } else {
                panic!("min_lat missing or wrong type");
            }
            if let Some(Value::Float(max_lat)) = obj.get("max_lat") {
                assert!(
                    (*max_lat - 1.0).abs() < 1e-10,
                    "max_lat should be 1.0, got {}",
                    max_lat
                );
            } else {
                panic!("max_lat missing or wrong type");
            }
            if let Some(Value::Float(min_lon)) = obj.get("min_lon") {
                assert!(
                    (*min_lon - 0.0).abs() < 1e-10,
                    "min_lon should be 0.0, got {}",
                    min_lon
                );
            } else {
                panic!("min_lon missing or wrong type");
            }
            if let Some(Value::Float(max_lon)) = obj.get("max_lon") {
                assert!(
                    (*max_lon - 1.0).abs() < 1e-10,
                    "max_lon should be 1.0, got {}",
                    max_lon
                );
            } else {
                panic!("max_lon missing or wrong type");
            }
        }
...
fn test_bounding_box_alias_returns_object() {
    ...
    match r.rows[0].data.get("bbox") {
        Some(Value::Object(obj)) => {
            ...
            if let Some(Value::Float(max_lat)) = obj.get("max_lat") {
                assert!(
                    (*max_lat - 30.0).abs() < 1e-10,
                    "max_lat should be 30.0, got {}",
                    max_lat
                );
            }
            if let Some(Value::Float(max_lon)) = obj.get("max_lon") {
                assert!(
                    (*max_lon - 40.0).abs() < 1e-10,
                    "max_lon should be 40.0, got {}",
                    max_lon
                );
            }
        }
    ...
    match r.rows[0].data.get("c") {
        Some(Value::Object(obj)) => {
            if let Some(Value::Float(lat)) = obj.get("lat") {
                assert!(
                    (*lat - 0.5).abs() < 1e-10,
                    "centroid lat should be 0.5, got {}",
                    lat
                );
            } else {
                panic!("lat missing or wrong type");
            }
            if let Some(Value::Float(lon)) = obj.get("lon") {
                assert!(
                    (*lon - 0.5).abs() < 1e-10,
                    "centroid lon should be 0.5, got {}",
                    lon
                );
            } else {
                panic!("lon missing or wrong type");
            }
        }
        other => panic!("expected Object, got {:?}", other),
    }
...
fn test_geo_area_km2_equator_degree_square() {
    ...
    match r.rows[0].data.get("area") {
        Some(Value::Float(f)) => {
            assert!(
                *f > 10000.0 && *f < 15000.0,
                "1-deg square at equator should be ~12392 km², got {}",
                f
            );
        }
...
fn test_polygon_area_too_few_points_returns_zero() {
    ...
    match r.rows[0].data.get("area") {
        Some(Value::Float(f)) => assert!(
            (*f).abs() < 1e-10,
            "expected 0 for 2-point polygon, got {}",
            f
        ),
...
fn test_geo_contains_alias() {
    ...
    assert_eq!(
        r.rows[0].data.get("inside"),
        Some(&Value::Bool(true)),
        "GEO_CONTAINS alias should return true"
    );
}
...
fn test_geo_perimeter_1_degree_square() {
    ...
    match r.rows[0].data.get("perim") {
        Some(Value::Float(f)) => {
            assert!(
                *f > 400.0 && *f < 500.0,
                "1-deg square perimeter should be ~445 km, got {}",
                f
            );
        }
        other => panic!("expected Float, got {:?}", other),
...
fn test_compass_bearing_north() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE b = COMPASS_BEARING(0.0, 0.0, 1.0, 0.0) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => {
            assert!(
                f.abs() < 1.0 || (*f - 360.0).abs() < 1.0,
                "due north bearing should be ~0 or ~360, got {}",
                f
            );
        }
        other => panic!("expected Float, got {:?}", other),
...
fn test_compass_bearing_east() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE b = COMPASS_BEARING(0.0, 0.0, 0.0, 1.0) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => {
            assert!(
...
fn test_compass_bearing_south() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE b = COMPASS_BEARING(1.0, 0.0, 0.0, 0.0) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - 180.0).abs() < 1.0,
                "due south bearing should be ~180, got {}",
                f
            );
        }
        other => panic!("expected Float, got {:?}", other),
...
fn test_geo_bearing_alias_result_in_range() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE b = COMPASS_BEARING(lat1, lon1, lat2, lon2) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => {
            assert!(
...
fn test_geo_destination_north_increases_lat() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE dest = GEO_PROJECT(0.0, 0.0, 0.0, 100.0) SELECT dest;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("dest") {
        Some(Value::Object(obj)) => {
            if let Some(Value::Float(lat)) = obj.get("lat") {
                ...
                panic!("lat missing or wrong type in destination");
            }
...
fn test_geo_project_alias() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE dest = GEO_PROJECT(0.0, 0.0, 90.0, 111.32) SELECT dest;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("dest") {
        Some(Value::Object(obj)) => {
            ...
            if let Some(Value::Float(lon)) = obj.get("lon") {
                assert!(
                    *lon > 0.5,
                    "traveling east 111km should increase lon, got {}",
                    lon
                );
            }
        }
...
fn test_geo_midpoint_same_point_returns_same() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE m = GEO_MIDPOINT_COORD(10.0, 20.0, 10.0, 20.0) SELECT m;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Object(obj)) => {
            if let Some(Value::Float(lat)) = obj.get("lat") {
                assert!(
                    (*lat - 10.0).abs() < 1e-6,
                    "midpoint lat should be 10.0, got {}",
                    lat
                );
            } else {
                panic!("lat missing or wrong type");
            ...
            if let Some(Value::Float(lon)) = obj.get("lon") {
                assert!(
                    (*lon - 20.0).abs() < 1e-6,
                    "midpoint lon should be 20.0, got {}",
                    lon
                );
            } else {
                panic!("lon missing or wrong type");
...
fn test_geo_midpoint_coord_alias() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE m = GEO_MIDPOINT_COORD(0.0, 0.0, 0.0, 90.0) SELECT m;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Object(obj)) => {
...
fn test_lines_intersect_alias() {
    ...
    assert_eq!(
        r.rows[0].data.get("x"),
        Some(&Value::Bool(true)),
        "LINES_INTERSECT alias should return true for crossing segments"
    );
}
...
fn test_geo_buffer_default_16_sides() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE poly = GEO_BUFFER(0.0, 0.0, 1.0) SELECT poly;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("poly") {
        Some(Value::Array(pts)) => {
            assert_eq!(
                pts.len(),
                16,
                "default GEO_BUFFER should produce 16 vertices, got {}",
                pts.len()
            );
        }
...
fn test_geo_buffer_custom_n_sides() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE poly = GEO_BUFFER(0.0, 0.0, 1.0, 8) SELECT poly;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("poly") {
        Some(Value::Array(pts)) => {
            assert_eq!(
                pts.len(),
                8,
                "GEO_BUFFER with n=8 should produce 8 vertices, got {}",
                pts.len()
            );
            // Each vertex should be an array of [lat, lon]
            match &pts[0] {
                Value::Array(coord) => {
                    assert_eq!(coord.len(), 2, "each vertex should have 2 coords")
                }
    ...
    match r.rows[0].data.get("poly") {
        Some(Value::Array(pts)) => {
            assert_eq!(
                pts.len(),
                32,
                "GEO_CIRCLE_POLYGON with n=32 should produce 32 vertices, got {}",
                pts.len()
            );
        }
        other => panic!("expected Array, got {:?}", other),
...
fn test_geo_buffer_vertices_near_center() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE poly = GEO_BUFFER(0.0, 0.0, 100.0, 4) SELECT poly;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("poly") {
        Some(Value::Array(pts)) => {
            ...
            for pt in pts {
                match pt {
                    Value::Array(coord) if coord.len() == 2 => {
                        let lat = match &coord[0] {
                            Value::Float(f) => *f,
                            _ => panic!("lat not float"),
                        };
                        let lon = match &coord[1] {
                            Value::Float(f) => *f,
                            _ => panic!("lon not float"),
                        };
