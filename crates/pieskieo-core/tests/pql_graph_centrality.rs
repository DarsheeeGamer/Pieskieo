...
fn test_graph_bfs_distance_same_node() {
    ...
    let id = Uuid::new_v4();
    db.put_doc_ns(None, Some("nodes"), id, serde_json::json!({"name": "A"}))
        .unwrap();
...
fn test_graph_bfs_distance_adjacent() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("nodes"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("nodes"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    ...
    assert_eq!(
        ...
        "adjacent nodes should be distance 1"
    );
}
...
fn test_graph_bfs_distance_two_hops() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("path2"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("path2"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("path2"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    assert_eq!(
        ...
        "two-hop path should be distance 2"
    );
}
...
fn test_graph_bfs_distance_unreachable() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("unreach"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("unreach"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    // No edges
...
fn test_bfs_distance_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("alias_bfs"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("alias_bfs"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
...
fn test_node_eccentricity_star_hub() {
    ...
    let l2 = Uuid::new_v4();
    let l3 = Uuid::new_v4();
    db.put_doc_ns(None, Some("star"), hub, serde_json::json!({"role": "hub"}))
        .unwrap();
    db.put_doc_ns(None, Some("star"), l1, serde_json::json!({"role": "leaf"}))
        .unwrap();
    db.put_doc_ns(None, Some("star"), l2, serde_json::json!({"role": "leaf"}))
        .unwrap();
    db.put_doc_ns(None, Some("star"), l3, serde_json::json!({"role": "leaf"}))
        .unwrap();
...
fn test_node_eccentricity_leaf_in_path() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("pathg"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("pathg"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("pathg"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(b, c, 1.0).unwrap();
    ...
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("e") {
        Some(Value::Integer(n)) => assert_eq!(
            *n, 2,
            "leaf eccentricity in path A-B-C should be 2, got {}",
            n
        ),
...
fn test_node_eccentricity_isolated_returns_null() {
    ...
    let ex = Executor::new(db.clone());
    let id = Uuid::new_v4();
    db.put_doc_ns(None, Some("iso"), id, serde_json::json!({"name": "solo"}))
        .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("e"),
        Some(&Value::Null),
        "isolated node eccentricity should be Null"
    );
}
...
fn test_common_neighbors_unrelated() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("noedge"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("noedge"), b, serde_json::json!({"name": "B"}))
        .unwrap();
...
fn test_shared_neighbors_alias() {
    ...
    db.put_doc_ns(None, Some("sn"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("sn"), shared, serde_json::json!({"name": "S"}))
        .unwrap();
    db.add_edge(a, shared, 1.0).unwrap();
    db.add_edge(b, shared, 1.0).unwrap();
    ...
    match r.rows[0].data.get("c") {
        Some(Value::Integer(n)) => {
            assert!(*n >= 1, "SHARED_NEIGHBORS alias should work, got {}", n)
        }
...
fn test_adamic_adar_score_alias_positive() {
    ...
    db.put_doc_ns(None, Some("aa"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("aa"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("aa"), mid, serde_json::json!({"name": "mid"}))
        .unwrap();
    db.put_doc_ns(None, Some("aa"), x, serde_json::json!({"name": "X"}))
        .unwrap();
    ...
    match r.rows[0].data.get("score") {
        Some(Value::Float(f)) => assert!(
            *f > 0.0,
            "ADAMIC_ADAR_SCORE should be positive with a common neighbor of degree > 1, got {}",
            f
        ),
...
fn test_adamic_adar_score_no_common_neighbors() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("aa2"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("aa2"), b, serde_json::json!({"name": "B"}))
        .unwrap();
...
fn test_jaccard_similarity_graph_no_neighbors() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("jac"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("jac"), b, serde_json::json!({"name": "B"}))
        .unwrap();
...
fn test_jaccard_similarity_graph_identical_neighbors() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("jac2"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("jac2"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("jac2"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    ...
    match r.rows[0].data.get("j") {
        Some(Value::Float(f)) => assert!(
            *f > 0.0 && *f <= 1.0,
            "Jaccard with shared neighbor should be in (0,1], got {}",
            f
        ),
...
fn test_graph_jaccard_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("jac3"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("jac3"), b, serde_json::json!({"name": "B"}))
        .unwrap();
...
fn test_katz_centrality_hub_higher_than_leaf() {
    ...
    let l2 = Uuid::new_v4();
    let l3 = Uuid::new_v4();
    db.put_doc_ns(None, Some("katz"), hub, serde_json::json!({"role": "hub"}))
        .unwrap();
    db.put_doc_ns(
        None,
        Some("katz"),
        leaf,
        serde_json::json!({"role": "leaf"}),
    )
    .unwrap();
    db.put_doc_ns(None, Some("katz"), l2, serde_json::json!({"role": "leaf"}))
        .unwrap();
    db.put_doc_ns(None, Some("katz"), l3, serde_json::json!({"role": "leaf"}))
        .unwrap();
    ...
    assert!(
        ...
        "hub Katz centrality {} should exceed leaf {}",
        hub_score,
        leaf_score
    );
}
...
fn test_katz_score_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("ks"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("ks"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    match r.rows[0].data.get("k") {
        Some(Value::Float(f)) => assert!(
            *f > 0.0,
            "KATZ_SCORE alias should return positive value, got {}",
            f
        ),
        other => panic!("expected Float, got {:?}", other),
    }
...
fn test_harmonic_centrality_connected_node() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("hc"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("hc"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("hc"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(b, c, 1.0).unwrap();
    ...
    match r.rows[0].data.get("h") {
        Some(Value::Float(f)) => assert!(
            *f > 0.0,
            "harmonic centrality should be positive for connected node, got {}",
            f
        ),
        other => panic!("expected positive Float, got {:?}", other),
    }
...
fn test_harmonic_centrality_isolated_node() {
    ...
    let ex = Executor::new(db.clone());
    let id = Uuid::new_v4();
    db.put_doc_ns(None, Some("hciso"), id, serde_json::json!({"name": "solo"}))
        .unwrap();
    ...
    assert_eq!(
        ...
        "isolated node harmonic centrality should be 0.0"
    );
}
...
fn test_harmonic_central_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("hca"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("hca"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
...
fn test_graph_dfs_reachable_path() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("dfs"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("dfs"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("dfs"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(b, c, 1.0).unwrap();
    ...
    match r.rows[0].data.get("n") {
        Some(Value::Integer(n)) => assert!(
            *n >= 2,
            "should reach at least 2 nodes from A in A-B-C, got {}",
            n
        ),
        other => panic!("expected Integer >= 2, got {:?}", other),
    }
...
fn test_graph_dfs_reachable_isolated() {
    ...
    assert_eq!(
        r.rows[0].data.get("n"),
        Some(&Value::Integer(0)),
        "isolated node should reach 0 other nodes"
    );
}
...
fn test_dfs_reachable_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("dfsa"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("dfsa"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    match r.rows[0].data.get("n") {
        Some(Value::Integer(n)) => {
            assert!(*n >= 1, "DFS_REACHABLE alias should return >= 1, got {}", n)
        }
        other => panic!("expected Integer, got {:?}", other),
    }
}
...
fn test_shortest_path_len_same_node() {
    ...
    let ex = Executor::new(db.clone());
    let id = Uuid::new_v4();
    db.put_doc_ns(None, Some("spl"), id, serde_json::json!({"name": "A"}))
        .unwrap();
...
fn test_shortest_path_len_connected() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("splc"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("splc"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("d"),
        Some(&Value::Integer(1)),
        "adjacent nodes SPL should be 1"
    );
}
...
fn test_spl_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("spla"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("spla"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
...
fn test_shortest_path_len_unreachable() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("splun"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("splun"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("d"),
        Some(&Value::Integer(-1)),
        "unreachable SPL should return -1"
    );
}
...
fn test_random_walk_returns_array() {
    ...
    let b = Uuid::new_v4();
    let c = Uuid::new_v4();
    db.put_doc_ns(None, Some("walk"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("walk"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("walk"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(b, c, 1.0).unwrap();
    ...
    match r.rows[0].data.get("w") {
        Some(Value::Array(arr)) => {
            ...
            assert!(
                arr.len() <= 4,
                "walk of 3 steps should have at most 4 elements, got {}",
                arr.len()
            );
            // First element should be the start node UUID string
            match &arr[0] {
                Value::String(s) => {
                    assert_eq!(s, &a.to_string(), "first element should be start node")
                }
    ...
    match r.rows[0].data.get("w") {
        Some(Value::Array(arr)) => {
            assert_eq!(
                arr.len(),
                1,
                "isolated node walk should have only start node, got {}",
                arr.len()
            );
        }
        other => panic!("expected Array, got {:?}", other),
...
fn test_graph_walk_alias_length() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("gwa"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("gwa"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    match r.rows[0].data.get("w") {
        Some(Value::Array(arr)) => {
            assert!(
                arr.len() <= n_steps + 1,
                "GRAPH_WALK of {} steps should have at most {} elements, got {}",
                n_steps,
                n_steps + 1,
                arr.len()
            );
