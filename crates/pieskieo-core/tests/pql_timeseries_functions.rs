...
fn test_moving_average() {
    ...
    let mut p = Parser::new(
        r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE ma = MOVING_AVERAGE(val, 3) SELECT ma;"#,
    );
        ...
        Some(Value::Array(arr)) => {
            ...
            for v in arr {
                assert!(
                    ...
                    "each moving average value should be a float"
                );
            }
        }
...
fn test_exponential_ma() {
    ...
    let mut p =
        Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE ema = EMA(val, 0.5) SELECT ema;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1);
            ...
            for v in arr {
                match v {
                    Value::Float(f) => {
                        assert!(
                            *f >= 10.0 && *f <= 50.0,
                            "EMA value {} should be between 10 and 50",
                            f
                        );
                    }
...
fn test_linear_trend() {
    ...
    let mut p = Parser::new(
        r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE slope = LINEAR_TREND(val) SELECT slope;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1);
    match r.rows[0].data.get("slope") {
        Some(Value::Float(f)) => assert!(
            f.abs() < 0.001,
            "slope of constant series should be 0.0, got {}",
            f
        ),
        other => panic!("expected float, got {:?}", other),
    }
...
fn test_delta() {
    ...
    let mut p =
        Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE d = DELTA(val) SELECT d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1);
    match r.rows[0].data.get("d") {
        Some(Value::Array(arr)) => {
            ...
            for v in arr {
                assert!(
                    matches!(v, Value::Float(_)),
                    "each delta value should be a float"
                );
            }
        }
        other => panic!("expected array, got {:?}", other),
...
fn test_normalize_series() {
    ...
    let mut p = Parser::new(
        r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE n = NORMALIZE_SERIES(val) SELECT n;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1);
...
fn test_detect_outliers() {
    ...
    let mut p = Parser::new(
        r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE out = DETECT_OUTLIERS(val, 2) SELECT out;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1);
