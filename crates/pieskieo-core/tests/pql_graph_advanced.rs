...
fn test_graph_triangle_count_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"na": [1,2,3], "nb": [2,3,4]}),
    )
    .unwrap();
...
fn test_local_clustering_coeff_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"deg": 4, "tri": 2}),
    )
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE res = LOCAL_CLUSTERING_COEFF(deg, tri) SELECT res;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("res") {
        Some(Value::Float(f)) => {
            assert!((f - 2.0 / 6.0).abs() < 1e-9, "expected ~0.333, got {}", f)
        }
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_node_strength_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"wts": [1.5, 2.5, 3.0]}),
    )
    .unwrap();
...
fn test_graph_assortativ_perfectly_correlated() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"na": [1,2,3,4], "nb": [1,2,3,4]}),
    )
    .unwrap();
...
fn test_degree_assortativ_alias_uncorrelated() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"na": [1,2,3,4], "nb": [4,3,2,1]}),
    )
    .unwrap();
...
fn test_neighbor_overlap_full() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"na": [1,2], "nb": [1,2,3]}),
    )
    .unwrap();
...
fn test_graph_triangle_count_full_overlap() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"na": [1,2,3], "nb": [1,2,3]}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE res = GRAPH_TRIANGLE_COUNT(na, nb) SELECT res;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
