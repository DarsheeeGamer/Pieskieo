...
fn test_cidr_contains_true() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "192.168.1.0/24", "ip": "192.168.1.5"}),
    )
    .unwrap();
...
fn test_cidr_contains_false() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "192.168.1.0/24", "ip": "192.168.2.1"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE result = CIDR_CONTAINS(ip, cidr) SELECT result;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_ip_in_cidr_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "10.0.0.0/8", "ip": "10.1.2.3"}),
    )
    .unwrap();
...
fn test_is_private_ip_192_168() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"ip": "192.168.0.1"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE result = IS_PRIVATE_IP(ip) SELECT result;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_cidr_net_addr_alias() {
    ...
    assert_eq!(
        r.rows[0].data.get("net"),
        Some(&Value::String("10.0.0.0".to_string()))
    );
    drop(dir);
}
...
fn test_cidr_bcast_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "172.16.0.0/12"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("bcast"),
        Some(&Value::String("172.31.255.255".to_string()))
    );
    drop(dir);
}
...
fn test_subnet_bits_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "192.168.0.0/16"}),
    )
    .unwrap();
...
fn test_is_mcast_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"ip": "239.255.255.255"}),
    )
    .unwrap();
...
fn test_is_valid_ipv4_false_out_of_range() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"ip": "999.999.999.999"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE result = IS_VALID_IPV4(ip) SELECT result;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_oui_prefix_alias_with_dashes() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"mac": "00-1A-2B-3C-4D-5E"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("oui"),
        Some(&Value::String("00:1A:2B".to_string()))
    );
    drop(dir);
}
...
fn test_mask_ip_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"ip": "10.20.30.40"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("anon"),
        Some(&Value::String("10.20.30.0".to_string()))
    );
    drop(dir);
}
...
fn test_well_known_port_alias() {
    ...
    assert_eq!(
        r.rows[0].data.get("name"),
        Some(&Value::String("dns".to_string()))
    );
    drop(dir);
}
