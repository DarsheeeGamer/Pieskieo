...
fn test_cond_true_branch() {
    ...
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE result = COND(flag, "pass", "fail") SELECT result;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("result"),
...
fn test_if_then_else_alias() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE result = IF_THEN_ELSE(active, "big", "small") SELECT result;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("result"),
...
fn test_array_map_passthrough() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE result = ARRAY_MAP(arr, arr) COMPUTE sz = ARRAY_COUNT(result) SELECT sz;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_array_filter_nulls() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"arr": [1, null, 2, false, 3]}),
    )
    .unwrap();
...
fn test_values_array_alias() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE result = VALUES_ARRAY(obj) COMPUTE sz = ARRAY_COUNT(result) SELECT sz;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_array_push() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE result = ARRAY_PUSH(arr, 4) COMPUTE sz = ARRAY_COUNT(result) SELECT sz;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_array_pop() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE result = ARRAY_POP(arr) COMPUTE sz = ARRAY_COUNT(result) SELECT sz;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sz"), Some(&Value::Integer(2)));
}
