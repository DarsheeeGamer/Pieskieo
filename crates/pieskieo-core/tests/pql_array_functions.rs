...
fn test_array_intersect_except() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": [1,2,3,4], "b": [3,4,5,6]}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("inter") {
        Some(Value::Array(arr)) => {
            assert_eq!(arr.len(), 2, "Intersection [1,2,3,4] ∩ [3,4,5,6] = [3,4]")
        }
...
fn test_array_deduplicate() {
    ...
    let mut p = Parser::new(
        "QUERY t COMPUTE d = ARRAY_DEDUPLICATE(arr) COMPUTE sz = ARRAY_COUNT(d) SELECT sz;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_array_compact() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"arr": [1, null, 2, null, 3]}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("r") {
        Some(Value::Array(arr)) => {
            assert_eq!(
                arr[0],
                Value::Integer(3),
                "After rotating [1,2,3,4,5] by 2, first element should be 3"
            );
        }
