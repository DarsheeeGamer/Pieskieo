...
fn test_is_number() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"n": 42, "s": "hello"}),
    )
    .unwrap();

    let mut p =
        Parser::new(r#"QUERY t COMPUTE a = IS_NUMBER(n) COMPUTE b = IS_NUMBER(s) SELECT a, b;"#);
...
fn test_is_string_bool_array_object() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "hello", "b": true, "arr": [1,2], "obj": {"x": 1}}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE is_s = IS_STRING(s) COMPUTE is_b = IS_BOOL(b) COMPUTE is_a = IS_ARRAY(arr) COMPUTE is_o = IS_OBJECT(obj) SELECT is_s, is_b, is_a, is_o;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_hex_conversion() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE h = TO_HEX(n) COMPUTE b = TO_BINARY(n) SELECT h, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("h"),
...
fn test_mask_functions() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"card": "1234567890123456"}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE m = MASK(card) COMPUTE pm = MASK_PARTIAL(card, 4, 4) SELECT m, pm;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_word_count_char_count() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"text": "hello world foo bar"}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE wc = WORD_COUNT(text) COMPUTE cc = CHAR_COUNT(text, "o") SELECT wc, cc;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_geo_point_functions() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE pt = GEO_POINT(lat, lon) COMPUTE miles = GEO_DISTANCE_MILES(lat, lon, 51.5074, -0.1278) SELECT pt, miles;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    ...
    match r.rows[0].data.get("miles") {
        Some(Value::Float(f)) => assert!(
            *f > 3000.0 && *f < 4000.0,
            "NYC-London should be ~3460 miles, got {}",
            f
        ),
...
fn test_range_agg_and_entropy() {
    ...
    let mut p =
        Parser::new(r#"QUERY data COMPUTE g = 1 GROUP BY g COMPUTE r = RANGE_AGG(val) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows.len(), 1);
    match result.rows[0].data.get("r") {
        Some(Value::Float(f)) => assert!(
            (*f - 40.0).abs() < 0.001,
            "range should be 50-10=40, got {}",
            f
        ),
...
fn test_iqr() {
    ...
    let mut p = Parser::new(
        r#"QUERY vals COMPUTE g = 1 GROUP BY g COMPUTE iqr_val = IQR(v) SELECT iqr_val;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows.len(), 1);
