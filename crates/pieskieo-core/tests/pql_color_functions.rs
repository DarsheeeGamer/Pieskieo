...
fn test_rgb_to_hex_green() {
    ...
    assert_eq!(
        r.rows[0].data.get("h"),
        Some(&Value::String("#00FF00".to_string()))
    );
}
...
fn test_rgb_to_hex_blue() {
    ...
    assert_eq!(
        r.rows[0].data.get("h"),
        Some(&Value::String("#0000FF".to_string()))
    );
}
...
fn test_color_to_hex_alias() {
    ...
    assert_eq!(
        r.rows[0].data.get("h"),
        Some(&Value::String("#FFA500".to_string()))
    );
}
...
fn test_color_to_hsl_alias() {
    ...
    match r.rows[0].data.get("hsl") {
        Some(Value::Object(m)) => {
            ...
            assert!(
                (h - 120.0).abs() < 1.0,
                "hue for green should be ~120, got {}",
                h
            );
        }
        other => panic!("expected Object, got {:?}", other),
...
fn test_rgb_to_hsl_white() {
    ...
    match r.rows[0].data.get("hsl") {
        Some(Value::Object(m)) => {
            ...
            assert!(
                (s - 0.0).abs() < 0.01,
                "saturation for white should be 0, got {}",
                s
            );
            assert!(
                (l - 1.0).abs() < 0.01,
                "lightness for white should be 1.0, got {}",
                l
            );
        }
        other => panic!("expected Object, got {:?}", other),
...
fn test_hsl_to_rgb_red() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"h": 0.0, "s": 1.0, "l": 0.5}),
    )
    .unwrap();
...
fn test_hsl_to_color_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"h": 240.0, "s": 1.0, "l": 0.5}),
    )
    .unwrap();
...
fn test_color_to_hsv_alias() {
    ...
    match r.rows[0].data.get("hsv") {
        Some(Value::Object(m)) => {
            ...
            assert!(
                (v - 0.0).abs() < 0.01,
                "value for black should be 0, got {}",
                v
            );
        }
        other => panic!("expected Object, got {:?}", other),
...
fn test_luminance_black() {
    ...
    assert!(
        (lum - 0.0).abs() < 0.0001,
        "luminance of black should be 0, got {}",
        lum
    );
}
...
fn test_luminance_white() {
    ...
    assert!(
        (lum - 1.0).abs() < 0.001,
        "luminance of white should be ~1.0, got {}",
        lum
    );
}
...
fn test_relative_luminance_alias() {
    ...
    assert!(
        lum > 0.2 && lum < 0.22,
        "luminance of red should be ~0.2126, got {}",
        lum
    );
}
...
fn test_contrast_ratio_black_white() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": [0, 0, 0], "b_col": [255, 255, 255]}),
    )
    .unwrap();
    ...
    assert!(
        (cr - 21.0).abs() < 0.1,
        "contrast ratio black/white should be ~21.0, got {}",
        cr
    );
}
...
fn test_color_contrast_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": [0, 0, 0], "b_col": [0, 0, 0]}),
    )
    .unwrap();
    ...
    assert!(
        (cr - 1.0).abs() < 0.01,
        "contrast ratio same colors should be 1.0, got {}",
        cr
    );
}
...
fn test_darken_red() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"c": [255, 0, 0], "amt": 0.1}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("dc") {
        Some(Value::Array(arr)) => {
            ...
            assert!(
                rv < 255.0,
                "darkened red R channel should be < 255, got {}",
                rv
            );
        }
...
fn test_darken_color_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"c": [200, 100, 50], "amt": 0.2}),
    )
    .unwrap();
...
fn test_lighten_black() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"c": [0, 0, 0], "amt": 0.1}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("lc") {
        Some(Value::Array(arr)) => {
            ...
            assert!(
                ...
                "lightened black should be non-zero"
            );
        }
        other => panic!("expected Array, got {:?}", other),
    }
...
fn test_lighten_color_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"c": [100, 100, 100], "amt": 0.2}),
    )
    .unwrap();
...
fn test_blend_colors_half() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": [255, 0, 0], "b_col": [0, 0, 255], "ratio": 0.5}),
    )
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE blended = BLEND_COLORS(a, b_col, ratio) SELECT blended;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_mix_colors_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": [100, 200, 50], "b_col": [50, 100, 150], "ratio": 0.5}),
    )
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE blended = MIX_COLORS(a, b_col, ratio) SELECT blended;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("blended") {
        Some(Value::Array(arr)) => {
...
fn test_grayscale_red() {
    ...
    match r.rows[0].data.get("g") {
        Some(Value::Integer(v)) => {
            assert!(
                *v >= 75 && *v <= 77,
                "grayscale of red should be ~76, got {}",
                v
            );
        }
...
fn test_complementary_color_red() {
    ...
    match r.rows[0].data.get("comp") {
        Some(Value::Array(arr)) => {
            ...
            assert!(
                gv > 250.0,
                "complementary of red: G should be ~255, got {}",
                gv
            );
            assert!(
                bv > 250.0,
                "complementary of red: B should be ~255, got {}",
                bv
            );
        }
        other => panic!("expected Array, got {:?}", other),
    ...
    match r.rows[0].data.get("rgb") {
        Some(Value::Array(arr)) => {
            ...
            for ch in arr {
                ...
                assert!(
                    v >= 0.0 && v <= 255.0,
                    "channel value should be in [0,255], got {}",
                    v
                );
            }
        }
