...
fn test_sigmoid_logistic_alias() {
    ...
    let mut p = Parser::new(r#"QUERY t2 COMPUTE a = SIGMOID(x) COMPUTE b = LOGISTIC(x) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    ...
    assert!((a - b).abs() < 1e-10, "SIGMOID and LOGISTIC should produce same result, got {} vs {}", a, b);
}
...
fn test_relu_positive_and_negative() {
    ...
    let mut p = Parser::new(r#"QUERY t3 COMPUTE rp = RELU(pos) COMPUTE rn = RELU(neg) SELECT rp, rn;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("rp") {
        Some(Value::Float(f)) => assert!(
            (*f - 5.0).abs() < 1e-9,
            "RELU(5) should be 5.0, got {}",
            f
        ),
...
fn test_mse_perfect_prediction() {
    ...
    let (_dir, _db, ex) = make_db("t6", serde_json::json!({"a": [1.0, 2.0, 3.0], "b": [1.0, 2.0, 3.0]}));
...
fn test_mse_known_value() {
    // MSE([0,0,0], [1,1,1]) = 1.0
    let (_dir, _db, ex) = make_db("t7", serde_json::json!({"a": [0.0, 0.0, 0.0], "b": [1.0, 1.0, 1.0]}));
...
fn test_rmse_known_value() {
    ...
    let (_dir, _db, ex) = make_db("t8", serde_json::json!({"a": [0.0, 0.0, 0.0, 0.0], "b": [2.0, 2.0, 2.0, 2.0]}));
...
fn test_r_squared_perfect_fit() {
    ...
    let (_dir, _db, ex) = make_db("t9", serde_json::json!({"a": [1.0, 2.0, 3.0, 4.0], "b": [1.0, 2.0, 3.0, 4.0]}));
...
fn test_feature_scale_boundaries() {
    ...
    let mut p = Parser::new(r#"QUERY t12 COMPUTE a = FEATURE_SCALE(lo, 0, 10) COMPUTE b = FEATURE_SCALE(hi, 0, 10) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    ...
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => assert!((*f - 1.0).abs() < 1e-9, "scale at max should be 1, got {}", f),
        other => panic!("expected Float for b, got {:?}", other),
    }
}
...
fn test_hinge_loss_correct_classification() {
    ...
    match r.rows[0].data.get("h") {
        Some(Value::Float(f)) => assert!(
            f.abs() < 1e-9,
            "HINGE_LOSS(1,2) should be 0.0, got {}",
            f
        ),
        other => panic!("expected Float, got {:?}", other),
    }
...
fn test_batch_accuracy_perfect() {
    ...
    let (_dir, _db, ex) = make_db("t15", serde_json::json!({"a": [1.0, 0.0, 1.0], "b": [1.0, 0.0, 1.0]}));
...
fn test_batch_accuracy_partial() {
    ...
    let (_dir, _db, ex) = make_db("t16", serde_json::json!({"a": [1.0, 1.0, 0.0, 0.0], "b": [1.0, 0.0, 1.0, 0.0]}));
...
fn test_mae_score_known_value() {
    ...
    let (_dir, _db, ex) = make_db("t20", serde_json::json!({"a": [0.0, 0.0, 0.0], "b": [1.0, 2.0, 3.0]}));
    let mut p = Parser::new(r#"QUERY t20 COMPUTE m = MAE_SCORE(a, b) SELECT m;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Float(f)) => assert!(
            (*f - 2.0).abs() < 1e-9,
            "MAE should be 2.0, got {}",
            f
        ),
        other => panic!("expected Float, got {:?}", other),
    }
...
fn test_cross_entropy_one_hot() {
    ...
    //                                        = -ln(0.8+eps) ≈ 0.2231
    let (_dir, _db, ex) = make_db("t22", serde_json::json!({"a": [0.0, 1.0, 0.0], "b": [0.1, 0.8, 0.1]}));
