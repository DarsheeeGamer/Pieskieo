...
fn test_json_has_path_exists() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"meta": {"active": true}}),
    )
    .unwrap();
...
fn test_json_set_path_top_level() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("x", x, "y", y) COMPUTE updated = JSON_SET_PATH(base, "/z", 99) COMPUTE sz = JSON_SIZE(updated) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_delete_path() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("a", a, "b", b, "c", c) COMPUTE trimmed = JSON_DELETE_PATH(base, "/b") COMPUTE sz = JSON_SIZE(trimmed) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_diff_changed_key() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE obj1 = JSON_BUILD_OBJECT("score", score) COMPUTE obj2 = JSON_BUILD_OBJECT("score", 20) COMPUTE d = JSON_DIFF(obj1, obj2) COMPUTE has_score = JSON_CONTAINS_KEY(d, "score") SELECT has_score;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_diff_no_diff() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 5}))
        .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE obj1 = JSON_BUILD_OBJECT("n", n) COMPUTE obj2 = JSON_BUILD_OBJECT("n", n) COMPUTE d = JSON_DIFF(obj1, obj2) COMPUTE sz = JSON_SIZE(d) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_compact() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"k": 1}))
        .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("k", k) COMPUTE s = JSON_COMPACT(base) SELECT s;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::String(s)) => {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(s).expect("compact output should be valid JSON");
...
fn test_json_pretty() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"k": 1}))
        .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("k", k) COMPUTE s = JSON_PRETTY(base) SELECT s;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::String(s)) => {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(s).expect("pretty output should be valid JSON");
            assert_eq!(parsed["k"], serde_json::json!(1));
        }
        other => panic!("expected String, got {:?}", other),
    }
...
fn test_json_keys_recursive() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"nested": {"inner": {"deep": 1}}}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("keys") {
        Some(Value::Array(a)) => {
            ...
            assert!(
                strs.contains(&"inner"),
                "should have 'inner', got {:?}",
                strs
            );
            assert!(
                strs.contains(&"inner.deep"),
                "should have 'inner.deep', got {:?}",
                strs
            );
        }
...
fn test_json_replace_existing_key() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("x", x, "y", y) COMPUTE updated = JSON_REPLACE(base, "/x", 99) COMPUTE v = JSON_POINTER(updated, "/x") SELECT v;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_replace_missing_key_no_add() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"x": 1}))
        .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("x", x) COMPUTE updated = JSON_REPLACE(base, "/newkey", 100) COMPUTE sz = JSON_SIZE(updated) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_arr_len_on_nested_array() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"container": {"nums": [1, 2, 3, 4]}}),
    )
    .unwrap();
...
fn test_remove_nulls() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("a", a, "b", null, "c", c) COMPUTE cleaned = REMOVE_NULLS(base) COMPUTE sz = JSON_SIZE(cleaned) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sz"), Some(&Value::Integer(2)));
}
...
fn test_json_select_keys() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"id": 1, "name": "Bob", "secret": "hidden"}),
    )
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("id", id, "name", name, "secret", secret) COMPUTE proj = JSON_SELECT_KEYS(base, ["id", "name"]) COMPUTE sz = JSON_SIZE(proj) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sz"), Some(&Value::Integer(2)));
}
...
fn test_json_select_keys_missing_key_skipped() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("a", a, "b", b) COMPUTE proj = JSON_SELECT_KEYS(base, ["a", "nonexistent"]) COMPUTE sz = JSON_SIZE(proj) SELECT sz;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_json_minify_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"v": 7}))
        .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE base = JSON_BUILD_OBJECT("v", v) COMPUTE s = JSON_MINIFY(base) SELECT s;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::String(s)) => {
            assert!(!s.is_empty());
            let parsed: serde_json::Value =
                serde_json::from_str(s).expect("minify output should be valid JSON");
