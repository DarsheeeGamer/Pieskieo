...
fn test_cosine_similarity() {
    ...
    match r.rows[0].data.get("sim") {
        Some(Value::Float(f)) => assert!(
            f.abs() < 1e-9,
            "orthogonal cosine sim should be 0, got {}",
            f
        ),
...
fn test_vector_normalize() {
    ...
    let mut p = Parser::new(
        "QUERY t COMPUTE n = VECTOR_NORMALIZE(v) COMPUTE mag = VECTOR_MAGNITUDE(n) SELECT mag;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("mag") {
        Some(Value::Float(f)) => assert!(
            (*f - 1.0).abs() < 1e-9,
            "normalized vector should have magnitude 1, got {}",
            f
        ),
        other => panic!("expected float, got {:?}", other),
    }
...
fn test_vector_add_and_scale() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE s = VECTOR_ADD(a, b) COMPUTE dim = VECTOR_DIM(s) SELECT dim;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
