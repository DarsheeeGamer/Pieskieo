...
fn test_topological_sort_dag() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = TOPOLOGICAL_SORT({"A": ["B", "C"], "B": ["C"], "C": []}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 3);
            let pos: std::collections::HashMap<String, usize> = arr
                .iter()
                .enumerate()
                .map(|(i, v)| {
...
fn test_topological_sort_linear() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = TOPOLOGICAL_SORT({"X": ["Y"], "Y": ["Z"], "Z": []}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 3);
...
fn test_topological_sort_cycle_returns_empty() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = TOPOLOGICAL_SORT({"A": ["B"], "B": ["C"], "C": ["A"]}) SELECT res;"#,
    );
    match v {
...
fn test_topo_sort_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = TOPO_SORT({"A": ["B"], "B": []}) SELECT res;"#,
    );
    match v {
...
fn test_topological_sort_dfs_dag() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = TOPO_SORT_DFS({"A": ["B", "C"], "B": ["D"], "C": ["D"], "D": []}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 4);
...
fn test_has_cycle_true() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = HAS_CYCLE({"A": ["B"], "B": ["C"], "C": ["A"]}) SELECT res;"#,
    );
...
fn test_has_cycle_false_dag() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = HAS_CYCLE({"A": ["B"], "B": ["C"], "C": []}) SELECT res;"#,
    );
...
fn test_has_cycle_single_node() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_HAS_CYCLE({"A": []}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(false));
}
...
fn test_bfs_order_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BFS_ORDER({"A": ["B", "C", "D"], "B": ["A"], "C": ["A"], "D": ["A"]}, "A") SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 4);
...
fn test_bfs_traversal_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BFS_TRAVERSAL({"A": ["B"], "B": ["C"], "C": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 3);
...
fn test_dfs_order_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DFS_ORDER({"A": ["B", "C"], "B": ["D"], "C": [], "D": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 4);
...
fn test_dfs_traversal_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DFS_TRAVERSAL({"A": ["B"], "B": ["C"], "C": []}, "A") SELECT res;"#,
    );
    match v {
...
fn test_bfs_layers_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BFS_LAYERS({"A": ["B"], "B": ["A", "C"], "C": ["B", "D"], "D": ["C"]}, "A") SELECT res;"#,
    );
    match v {
        Value::Array(layers) => {
            assert_eq!(layers.len(), 4, "4 BFS levels for a chain of 4");
            if let Value::Array(l0) = &layers[0] {
                assert_eq!(l0.len(), 1);
            }
            if let Value::Array(l1) = &layers[1] {
                assert_eq!(l1.len(), 1);
            }
        }
        other => panic!("expected array, got {:?}", other),
...
fn test_bfs_levels_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BFS_LEVELS({"A": ["B", "C"], "B": [], "C": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Array(layers) => {
...
fn test_is_connected_true() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = IS_CONNECTED({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_is_connected_false() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = IS_GRAPH_CONNECTED({"A": ["B"], "B": ["A"], "C": ["D"], "D": ["C"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(false));
}
...
fn test_connected_components_two() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = CONNECTED_COMPONENTS({"A": ["B"], "B": ["A"], "C": ["D"], "D": ["C"]}) SELECT res;"#,
    );
    match v {
...
fn test_weakly_connected_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = WEAKLY_CONNECTED({"A": ["B"], "B": ["A"], "C": []}) SELECT res;"#,
    );
    match v {
...
fn test_scc_simple() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = SCC({"A": ["B"], "B": ["C"], "C": ["A"], "D": ["A"]}) SELECT res;"#,
    );
    match v {
        Value::Array(sccs) => {
...
fn test_strongly_connected_components_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = STRONGLY_CONNECTED_COMPONENTS({"A": ["B"], "B": ["A"]}) SELECT res;"#,
    );
    match v {
...
fn test_is_bipartite_true() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = IS_BIPARTITE({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_is_bipartite_false_triangle() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_IS_BIPARTITE({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(false));
}
...
fn test_is_eulerian_true() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = IS_EULERIAN({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_has_euler_circuit_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = HAS_EULER_CIRCUIT({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_is_semi_eulerian_path_exists() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = IS_SEMI_EULERIAN({"A": ["B"], "B": ["A", "C"], "C": ["B", "D"], "D": ["C"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_has_euler_path_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = HAS_EULER_PATH({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Bool(true));
}
...
fn test_articulation_points_bridge_graph() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = ARTICULATION_POINTS({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert!(
                arr.iter().any(|x| x == &Value::String("B".to_string())),
                "B should be AP, got {:?}",
                arr
            );
        }
...
fn test_cut_vertices_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = CUT_VERTICES({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    match v {
...
fn test_articulation_points_none_in_triangle() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = ARTICULATION_POINTS({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B"]}) SELECT res;"#,
    );
    match v {
...
fn test_bridge_edges_single_bridge() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BRIDGE_EDGES({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B", "D"], "D": ["C"]}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
...
fn test_graph_bridges_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_BRIDGES({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    match v {
...
fn test_dijkstra_dist_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DIJKSTRA_DIST({"A": [["B", 1.0], ["C", 4.0]], "B": [["C", 2.0]], "C": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Object(map) => {
...
fn test_shortest_dist_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = SHORTEST_DIST({"A": [["B", 2.0]], "B": [["C", 3.0]], "C": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Object(map) => {
...
fn test_dijkstra_path_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DIJKSTRA_PATH({"A": [["B", 1.0], ["C", 10.0]], "B": [["C", 1.0]], "C": []}, "A", "C") SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
...
fn test_shortest_path_nodes_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = SHORTEST_PATH_NODES({"A": [["B", 5.0]], "B": []}, "A", "B") SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 2);
...
fn test_dijkstra_path_no_path() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DIJKSTRA_PATH({"A": [], "B": []}, "A", "B") SELECT res;"#,
    );
    match v {
...
fn test_bellman_ford_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BELLMAN_FORD({"A": [["B", 1.0], ["C", 4.0]], "B": [["C", 2.0]], "C": []}, "A") SELECT res;"#,
    );
    match v {
        Value::Object(map) => {
            assert_eq!(map.get("A"), Some(&Value::Float(0.0)));
...
fn test_bellman_ford_dist_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = BELLMAN_FORD_DIST({"A": [["B", 3.0]], "B": []}, "A") SELECT res;"#,
    );
    match v {
...
fn test_apsp_floyd_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = APSP_FLOYD({"A": [["B", 1.0]], "B": [["C", 2.0]], "C": []}) SELECT res;"#,
    );
    match v {
        Value::Object(outer) => {
...
fn test_all_pairs_shortest_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = ALL_PAIRS_SHORTEST({"P": [["Q", 5.0]], "Q": []}) SELECT res;"#,
    );
    match v {
        Value::Object(outer) => {
...
fn test_max_flow_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MAX_FLOW({"S": [["A", 3.0], ["B", 2.0]], "A": [["T", 2.0]], "B": [["T", 3.0]], "T": []}, "S", "T") SELECT res;"#,
    );
    match v {
...
fn test_max_flow_bfs_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MAX_FLOW_BFS({"S": [["T", 10.0]], "T": []}, "S", "T") SELECT res;"#,
    );
    match v {
...
fn test_min_cut_equals_max_flow() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MIN_CUT({"S": [["A", 3.0], ["B", 2.0]], "A": [["T", 2.0]], "B": [["T", 3.0]], "T": []}, "S", "T") SELECT res;"#,
    );
    match v {
...
fn test_min_cut_value_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MIN_CUT_VALUE({"S": [["T", 7.0]], "T": []}, "S", "T") SELECT res;"#,
    );
    match v {
...
fn test_flow_network_capacity_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = FLOW_NETWORK_CAPACITY({"S": [["A", 3.0], ["B", 2.0]], "A": [], "B": []}, "S") SELECT res;"#,
    );
    match v {
        Value::Float(f) => assert!(
            (f - 5.0).abs() < 1e-9,
            "total capacity should be 5, got {}",
            f
        ),
        other => panic!("expected float, got {:?}", other),
    }
...
fn test_network_capacity_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = NETWORK_CAPACITY({"X": [["Y", 10.0]], "Y": []}, "X") SELECT res;"#,
    );
    match v {
...
fn test_eccentricity_path() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = ECCENTRICITY({"A": ["B"], "B": ["A", "C"], "C": ["B"]}, "A") SELECT res;"#,
    );
...
fn test_node_eccentricity_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = NODE_ECCENTRICITY({"A": ["B"], "B": ["A", "C"], "C": ["B"]}, "B") SELECT res;"#,
    );
...
fn test_graph_diameter_path() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_DIAMETER({"A": ["B"], "B": ["A", "C"], "C": ["B", "D"], "D": ["C"]}) SELECT res;"#,
    );
...
fn test_diameter_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = DIAMETER({"A": ["B"], "B": ["A"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Integer(1));
}
...
fn test_graph_radius_path() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_RADIUS({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Integer(1));
}
...
fn test_radius_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = RADIUS({"A": ["B"], "B": ["A"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Integer(1));
}
...
fn test_graph_center_nodes_path() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = GRAPH_CENTER_NODES({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    match v {
        Value::Array(arr) => {
            assert_eq!(arr.len(), 1);
...
fn test_center_nodes_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = CENTER_NODES({"A": ["B"], "B": ["A"]}) SELECT res;"#,
    );
    match v {
...
fn test_mst_kruskal_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MST_KRUSKAL({"A": [["B", 1.0], ["C", 4.0]], "B": [["A", 1.0], ["C", 2.0]], "C": [["A", 4.0], ["B", 2.0]]}) SELECT res;"#,
    );
    match v {
        Value::Object(obj) => {
            match obj.get("total_weight") {
                Some(Value::Float(f)) => {
                    assert!((f - 3.0).abs() < 1e-9, "MST weight should be 3, got {}", f)
                }
                other => panic!("expected float for total_weight, got {:?}", other),
            }
            match obj.get("edges") {
                Some(Value::Array(edges)) => {
                    assert_eq!(edges.len(), 2, "MST of 3-node graph has 2 edges")
                }
...
fn test_min_spanning_tree_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MIN_SPANNING_TREE({"A": [["B", 5.0]], "B": [["A", 5.0]]}) SELECT res;"#,
    );
    match v {
        Value::Object(obj) => match obj.get("total_weight") {
...
fn test_mst_weight_basic() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MST_WEIGHT({"A": [["B", 1.0], ["C", 4.0]], "B": [["A", 1.0], ["C", 2.0]], "C": [["A", 4.0], ["B", 2.0]]}) SELECT res;"#,
    );
    match v {
...
fn test_minimum_spanning_weight_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = MINIMUM_SPANNING_WEIGHT({"A": [["B", 7.0]], "B": [["A", 7.0]]}) SELECT res;"#,
    );
    match v {
        Value::Float(f) => assert!((f - 7.0).abs() < 1e-9),
        other => panic!("expected float, got {:?}", other),
...
fn test_spanning_tree_count_k3() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = SPANNING_TREE_COUNT({"A": ["B", "C"], "B": ["A", "C"], "C": ["A", "B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Integer(3));
}
...
fn test_num_spanning_trees_alias() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = NUM_SPANNING_TREES({"A": ["B"], "B": ["A", "C"], "C": ["B"]}) SELECT res;"#,
    );
    assert_eq!(v, Value::Integer(1));
}
...
fn test_spanning_tree_count_k4() {
    ...
    let v = run(
        &ex,
        r#"QUERY t COMPUTE res = SPANNING_TREE_COUNT({"A": ["B","C","D"], "B": ["A","C","D"], "C": ["A","B","D"], "D": ["A","B","C"]}) SELECT res;"#,
    );
