use pieskieo_core::{PieskieoDb, pql::{Executor, Parser}};
use std::sync::Arc;
...
fn test_copy_to_csv_and_from_csv() {
    ...
    for (name, age) in [("Alice", 30), ("Bob", 25)] {
        db.put_doc_ns(None, Some("people"), Uuid::new_v4(),
            serde_json::json!({"name": name, "age": age})).unwrap();
    }
    // Copy to CSV
    let csv_path = dir.path().join("people.csv");
    let csv_path_str = csv_path.to_string_lossy().to_string();
    let mut p = Parser::new(&format!("COPY people TO '{}' FORMAT CSV HEADER true;", csv_path_str));
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows.len(), 1); // status row
    assert!(csv_path.exists());

    // Copy from CSV into new collection
    let mut p2 = Parser::new(&format!("COPY people2 FROM '{}' FORMAT CSV HEADER true;", csv_path_str));
...
fn test_copy_to_json_and_from_json() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("items"), Uuid::new_v4(),
        serde_json::json!({"id": 1, "label": "widget"})).unwrap();
    ...
    let mut p2 = Parser::new(&format!("COPY items2 FROM '{}' FORMAT JSON;", json_path_str));
