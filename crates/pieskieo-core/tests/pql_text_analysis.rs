...
fn test_sentiment_score_positive() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "This is great and wonderful and awesome"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE s = SENTIMENT_SCORE(txt) SELECT s;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f > 0.0, "positive text should have positive score, got {}", f),
...
fn test_sentiment_score_negative() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "This is terrible and horrible and awful"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE s = POLARITY_SCORE(txt) SELECT s;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f < 0.0, "negative text should have negative score, got {}", f),
        other => panic!("expected float, got {:?}", other),
    }
}
...
fn test_language_detect_english() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "the quick brown fox and the dog are running"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE lang = LANGUAGE_DETECT(txt) SELECT lang;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("lang"), Some(&Value::String("en".to_string())));
}
...
fn test_language_detect_french() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "le chat et la souris sont dans le jardin"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE lang = DETECT_LANGUAGE(txt) SELECT lang;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("lang"), Some(&Value::String("fr".to_string())));
}
...
fn test_extract_emails() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "Contact us at hello@example.com or support@foo.org for help"})).unwrap();
...
fn test_extract_numbers() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "The price is 42.50 and quantity is 3"})).unwrap();
...
fn test_extract_hashtags() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "#hello world #foo_bar check #rust2024"})).unwrap();
...
fn test_extract_mentions() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "Hey @alice and @bob_smith, how are you?"})).unwrap();
...
fn test_extract_phone_numbers() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "Call us at (555) 123-4567 or 800-555-1234"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE phones = EXTRACT_PHONE_NUMBERS(txt) SELECT phones;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("phones") {
        Some(Value::Array(arr)) => {
            assert!(!arr.is_empty(), "expected at least one phone number, got: {:?}", arr);
        }
        other => panic!("expected array, got {:?}", other),
    }
}
...
fn test_remove_stopwords() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "the quick brown fox is jumping over a lazy dog"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE filtered = REMOVE_STOPWORDS(txt) SELECT filtered;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("filtered") {
        Some(Value::String(s)) => {
            assert!(!s.contains("the "), "stopword 'the' should be removed, got: {}", s);
            assert!(!s.contains(" is "), "stopword 'is' should be removed, got: {}", s);
            assert!(s.contains("quick"), "content word 'quick' should remain, got: {}", s);
            assert!(s.contains("fox"), "content word 'fox' should remain, got: {}", s);
        }
...
fn test_keyword_extract() {
    ...
    match r.rows[0].data.get("kws") {
        Some(Value::Array(arr)) => {
            assert!(arr.len() <= 3, "expected at most 3 keywords, got: {:?}", arr);
            assert!(!arr.is_empty(), "should return some keywords");
            // "rust" appears 3 times, should be top keyword
            assert!(arr.contains(&Value::String("rust".to_string())), "rust should be a top keyword, got: {:?}", arr);
        }
        other => panic!("expected array, got {:?}", other),
    }
}
...
fn test_abbreviate() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "Natural Language Processing"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE abbr = ABBREVIATE(txt) SELECT abbr;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("abbr"), Some(&Value::String("NLP".to_string())));
}
...
fn test_expand_contractions() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "I don't know what he's doing"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE expanded = EXPAND_CONTRACTIONS(txt) SELECT expanded;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("expanded") {
        Some(Value::String(s)) => {
            assert!(s.contains("do not"), "don't should expand to 'do not', got: {}", s);
            assert!(s.contains("he is"), "he's should expand to 'he is', got: {}", s);
        }
        other => panic!("expected string, got {:?}", other),
    }
}
...
fn test_clean_text() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "<p>Hello  <b>world</b>!   This is   clean.</p>"})).unwrap();
    ...
    match r.rows[0].data.get("cleaned") {
        Some(Value::String(s)) => {
            ...
            assert!(!s.contains('>'), "HTML tags should be removed, got: {}", s);
            assert!(!s.contains("  "), "Multiple spaces should be normalized, got: {}", s);
...
fn test_count_tokens() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "one two three four five six seven eight nine ten"})).unwrap();
...
fn test_find_hashtags_alias() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "#openai #gpt4"})).unwrap();
...
fn test_initialism_alias() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"txt": "Application Programming Interface"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE abbr = INITIALISM(txt) SELECT abbr;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("abbr"), Some(&Value::String("API".to_string())));
}
