...
fn test_regexp_count() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"text": "hello world hello foo hello"}),
    )
    .unwrap();
...
fn test_regexp_extract() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "price: $42.50"}),
    )
    .unwrap();
    // PQL string: "[0-9]+\\.[0-9]+" -> regex [0-9]+\.[0-9]+
    let mut p =
        Parser::new(r#"QUERY t COMPUTE n = REGEXP_EXTRACT(s, "[0-9]+\\.[0-9]+") SELECT n;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("n"),
        Some(&Value::String("42.50".to_string()))
    );
}
...
fn test_regexp_extract_all() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "cat bat sat"}),
    )
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE matches = REGEXP_EXTRACT_ALL(s, "[a-z]at") COMPUTE cnt = ARRAY_COUNT(matches) SELECT cnt;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_split_regex() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "one  two   three"}),
    )
    .unwrap();
