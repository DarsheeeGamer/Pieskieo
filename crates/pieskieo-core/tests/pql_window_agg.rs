...
fn make_db_with(ns: &str, docs: Vec<serde_json::Value>) -> (tempfile::TempDir, Arc<PieskieoDb>, Executor) {
...
fn test_cume_dist_returns_float() {
    ...
    let (_dir, _db, ex) = make_db_with("test_window_cd", vec![
...
fn test_percent_rank_returns_float() {
    ...
    let (_dir, _db, ex) = make_db_with("test_window_pr", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_first_value_returns_null_without_context() {
    let (_dir, _db, ex) = make_db_with("test_window_fv", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_last_value_returns_null_without_context() {
    let (_dir, _db, ex) = make_db_with("test_window_lv", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_nth_value_returns_null_without_context() {
    let (_dir, _db, ex) = make_db_with("test_window_nv", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_lag_returns_null_without_context() {
    let (_dir, _db, ex) = make_db_with("test_window_lag", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_lag_default_value() {
    let (_dir, _db, ex) = make_db_with("test_window_lag2", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
    // LAG with explicit default -1 should return -1 when no marker exists
    let r = run(&ex, "QUERY test_window_lag2 SELECT LAG(val, 1, -1) AS prev_val;");
    assert!(!r.rows.is_empty());
...
fn test_lead_returns_null_without_context() {
    let (_dir, _db, ex) = make_db_with("test_window_lead", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
...
fn test_lead_default_value() {
    let (_dir, _db, ex) = make_db_with("test_window_lead2", vec![
        serde_json::json!({"id": 1, "val": 42}),
    ]);
    let r = run(&ex, "QUERY test_window_lead2 SELECT LEAD(val, 1, 999) AS next_val;");
    assert!(!r.rows.is_empty());
...
fn test_every_single_true_row() {
    let (_dir, _db, ex) = make_db_with("test_every_t", vec![
...
fn test_every_single_false_row() {
    let (_dir, _db, ex) = make_db_with("test_every_f", vec![
        serde_json::json!({"id": 1, "active": false}),
    ]);
...
fn test_every_group_all_true() {
    ...
    let (_dir, _db, ex) = make_db_with("test_every_grp", vec![
        serde_json::json!({"grp": 1, "active": true}),
        serde_json::json!({"grp": 1, "active": true}),
        serde_json::json!({"grp": 1, "active": true}),
    ]);
    let r = run(&ex, "QUERY test_every_grp COMPUTE g = grp GROUPBY g COMPUTE ea = EVERY(active) SELECT ea;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("ea") {
        Some(Value::Bool(true)) => {}
        other => panic!("Expected Bool(true) for EVERY on all-true group, got {:?}", other),
    }
}
...
fn test_covar_samp_null_single_row() {
    ...
    let (_dir, _db, ex) = make_db_with("test_covar_samp1", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
    ]);
    let r = run(&ex, "QUERY test_covar_samp1 COMPUTE g = 1 GROUPBY g COMPUTE cv = COVAR_SAMP(y, x) SELECT cv;");
    assert!(!r.rows.is_empty());
...
fn test_covar_samp_two_rows() {
    ...
    let (_dir, _db, ex) = make_db_with("test_covar_samp2", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 3.0, "y": 4.0}),
    ]);
    let r = run(&ex, "QUERY test_covar_samp2 COMPUTE g = 1 GROUPBY g COMPUTE cv = COVAR_SAMP(y, x) SELECT cv;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("cv") {
        Some(Value::Float(f)) => {
            assert!((f - 2.0).abs() < 1e-9, "Expected COVAR_SAMP = 2.0, got {}", f);
        }
...
fn test_regr_count_group() {
    let (_dir, _db, ex) = make_db_with("test_regr_cnt", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 3.0, "y": 4.0}),
        serde_json::json!({"x": 5.0, "y": 6.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_cnt COMPUTE g = 1 GROUPBY g COMPUTE cnt = REGR_COUNT(y, x) SELECT cnt;");
    assert!(!r.rows.is_empty());
...
fn test_regr_count_single_row() {
    let (_dir, _db, ex) = make_db_with("test_regr_cnt1", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
    ]);
...
fn test_regr_r2_perfect_correlation() {
    ...
    let (_dir, _db, ex) = make_db_with("test_regr_r2", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 2.0, "y": 4.0}),
        serde_json::json!({"x": 3.0, "y": 6.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_r2 COMPUTE g = 1 GROUPBY g COMPUTE r2 = REGR_R2(y, x) SELECT r2;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("r2") {
        Some(Value::Float(f)) => {
            assert!((f - 1.0).abs() < 1e-6, "Expected R² = 1.0 for perfect correlation, got {}", f);
        }
...
fn test_regr_avgx_group() {
    let (_dir, _db, ex) = make_db_with("test_regr_avgx", vec![
        serde_json::json!({"x": 1.0, "y": 1.0}),
        serde_json::json!({"x": 3.0, "y": 1.0}),
        serde_json::json!({"x": 5.0, "y": 1.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_avgx COMPUTE g = 1 GROUPBY g COMPUTE ax = REGR_AVGX(y, x) SELECT ax;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("ax") {
        Some(Value::Float(f)) => {
            assert!((f - 3.0).abs() < 1e-9, "Expected REGR_AVGX = 3.0, got {}", f);
        }
...
fn test_regr_avgy_group() {
    let (_dir, _db, ex) = make_db_with("test_regr_avgy", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 1.0, "y": 4.0}),
        serde_json::json!({"x": 1.0, "y": 6.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_avgy COMPUTE g = 1 GROUPBY g COMPUTE ay = REGR_AVGY(y, x) SELECT ay;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("ay") {
        Some(Value::Float(f)) => {
            assert!((f - 4.0).abs() < 1e-9, "Expected REGR_AVGY = 4.0, got {}", f);
        }
...
fn test_regr_sxx_group() {
    ...
    let (_dir, _db, ex) = make_db_with("test_regr_sxx", vec![
        serde_json::json!({"x": 1.0, "y": 1.0}),
        serde_json::json!({"x": 2.0, "y": 1.0}),
        serde_json::json!({"x": 3.0, "y": 1.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_sxx COMPUTE g = 1 GROUPBY g COMPUTE sxx = REGR_SXX(y, x) SELECT sxx;");
    assert!(!r.rows.is_empty());
...
fn test_regr_syy_group() {
    ...
    let (_dir, _db, ex) = make_db_with("test_regr_syy", vec![
        serde_json::json!({"x": 1.0, "y": 1.0}),
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 1.0, "y": 3.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_syy COMPUTE g = 1 GROUPBY g COMPUTE syy = REGR_SYY(y, x) SELECT syy;");
    assert!(!r.rows.is_empty());
...
fn test_regr_sxy_group() {
    ...
    let (_dir, _db, ex) = make_db_with("test_regr_sxy", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 2.0, "y": 4.0}),
        serde_json::json!({"x": 3.0, "y": 6.0}),
    ]);
    let r = run(&ex, "QUERY test_regr_sxy COMPUTE g = 1 GROUPBY g COMPUTE sxy = REGR_SXY(y, x) SELECT sxy;");
    assert!(!r.rows.is_empty());
...
fn test_xmlagg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_xmlagg1", vec![
        serde_json::json!({"id": 1, "data": "hello"}),
    ]);
...
fn test_xmlagg_group() {
    let (_dir, _db, ex) = make_db_with("test_xmlagg2", vec![
...
fn test_jsonb_object_agg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_jboagg1", vec![
        serde_json::json!({"id": 1, "k": "color", "v": "blue"}),
    ]);
    let r = run(&ex, "QUERY test_jboagg1 SELECT JSONB_OBJECT_AGG(k, v) AS obj;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("obj") {
        Some(Value::Object(m)) => {
            assert!(m.contains_key("color"), "Expected key 'color' in JSONB_OBJECT_AGG result");
        }
...
fn test_jsonb_object_agg_group() {
    let (_dir, _db, ex) = make_db_with("test_jboagg2", vec![
    ...
    match r.rows[0].data.get("obj") {
        Some(Value::Object(m)) => {
            assert!(m.len() >= 1, "Expected at least one key in JSONB_OBJECT_AGG result");
        }
        other => panic!("Expected Object for JSONB_OBJECT_AGG group, got {:?}", other),
    }
}
...
fn test_string_agg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_sagg1", vec![
        serde_json::json!({"id": 1, "name": "alice"}),
    ]);
    let r = run(&ex, "QUERY test_sagg1 SELECT STRING_AGG(name, ',') AS names;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("names") {
        Some(Value::String(s)) => {
            assert!(s.contains("alice"), "Expected 'alice' in STRING_AGG result, got: {}", s);
        }
...
fn test_array_agg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_aagg1", vec![
        serde_json::json!({"id": 1, "val": 10}),
    ]);
...
fn test_bit_and_group() {
    ...
    let (_dir, _db, ex) = make_db_with("test_bitand_g", vec![
        serde_json::json!({"grp": 1, "flags": 7}),
        serde_json::json!({"grp": 1, "flags": 5}),
        serde_json::json!({"grp": 1, "flags": 3}),
    ]);
    let r = run(&ex, "QUERY test_bitand_g COMPUTE g = grp GROUPBY g COMPUTE f = BIT_AND_AGG(flags) SELECT f;");
    assert!(!r.rows.is_empty());
...
fn test_bool_and_passthrough() {
    ...
    let (_dir, _db, ex) = make_db_with("test_bools_p", vec![
        serde_json::json!({"grp": 1, "active": true}),
        serde_json::json!({"grp": 1, "active": true}),
    ]);
    ...
    match r.rows[0].data.get("all_active") {
        Some(Value::Bool(true)) => {}
        other => panic!("Expected Bool(true) for BOOL_AND on all-true group, got {:?}", other),
    }
}
...
fn test_covar_pop_returns_float() {
    let (_dir, _db, ex) = make_db_with("test_cov_p", vec![
        serde_json::json!({"x": 1.0, "y": 2.0}),
        serde_json::json!({"x": 3.0, "y": 4.0}),
    ]);
    let r = run(&ex, "QUERY test_cov_p COMPUTE g = 1 GROUPBY g COMPUTE cov = COVAR_POP(y, x) SELECT cov;");
    assert!(!r.rows.is_empty());
...
fn test_json_agg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_jagg1", vec![
        serde_json::json!({"id": 1, "data": "hello"}),
    ]);
...
fn test_json_object_agg_single_row() {
    let (_dir, _db, ex) = make_db_with("test_joagg1", vec![
        serde_json::json!({"id": 1, "k": "key", "v": "val"}),
    ]);
    let r = run(&ex, "QUERY test_joagg1 SELECT OBJECT_AGG(k, v) AS obj;");
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("obj") {
        Some(Value::Object(m)) => {
            assert!(m.contains_key("key"), "Expected 'key' in JSON_OBJECT_AGG result");
        }
