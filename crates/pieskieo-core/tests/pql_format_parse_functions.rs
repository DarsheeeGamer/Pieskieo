...
fn test_parse_int_basic() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "  42  ", "bad": "abc"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE n = PARSE_INT(s) COMPUTE m = PARSE_INT(bad) SELECT n, m;"#);
...
fn test_parse_int_hex() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"hex_val": "0xff"})).unwrap();
...
fn test_parse_float_basic() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"num_str": "  3.14  ", "bad": "xyz"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE f = PARSE_FLOAT(num_str) COMPUTE g = PARSE_FLOAT(bad) SELECT f, g;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_parse_date_str() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "15/03/2024"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = PARSE_DATE_STR(dt, "%d/%m/%Y") SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("2024-03-15".to_string())));
}
...
fn test_parse_date_str_invalid() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "not-a-date"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = PARSE_DATE_STR(dt, "%d/%m/%Y") SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_parse_csv_line_simple() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"csv_line": "a,b,c"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE arr = PARSE_CSV_LINE(csv_line) SELECT arr;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("arr"), Some(&Value::Array(vec![
...
fn test_parse_csv_line_quoted() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"csv_line": "a,\"b,c\",d"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE arr = PARSE_CSV_LINE(csv_line) SELECT arr;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("arr"), Some(&Value::Array(vec![
        Value::String("a".to_string()),
...
fn test_parse_kv_ampersand() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"kvstr": "name=John&age=30"})).unwrap();
...
fn test_parse_tsv_line() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tsv_line": "alpha\tbeta\tgamma"})).unwrap();
...
fn test_format_bytes_mb() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": 1048576})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_BYTES(n) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_format_bytes_partial() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": 1536})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_BYTES(n) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("1.5 KB".to_string())));
}
...
fn test_format_duration_basic() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"secs": 3661})).unwrap();
...
fn test_format_duration_subsecond() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"secs": 1.5})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_DURATION(secs) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("1s 500ms".to_string())));
}
...
fn test_format_percent_default() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"ratio": 0.15})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_PERCENT(ratio) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("15.0%".to_string())));
}
...
fn test_format_percent_custom_decimals() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"ratio": 0.153})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_PERCENT(ratio, 2) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("15.30%".to_string())));
}
...
fn test_format_currency_usd() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"amt": 1234567.89})).unwrap();
...
fn test_format_currency_eur() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"amt": 1000.0})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = FORMAT_CURRENCY(amt, "EUR") SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("\u{20ac}1,000.00".to_string())));
}
...
fn test_format_scientific_basic() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": 0.00123})).unwrap();
    ...
    match r.rows[0].data.get("out") {
        Some(Value::String(s)) => {
            ...
            assert!(s.contains("e-") || s.contains("e+"), "expected scientific notation, got: {}", s);
        }
...
fn test_format_ordinal() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"a": 1, "b": 2, "c": 3, "d": 11, "e": 12, "f": 21})).unwrap();
    let mut p = Parser::new(r#"QUERY t
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("o1"), Some(&Value::String("1st".to_string())));
    assert_eq!(r.rows[0].data.get("o2"), Some(&Value::String("2nd".to_string())));
    assert_eq!(r.rows[0].data.get("o3"), Some(&Value::String("3rd".to_string())));
    assert_eq!(r.rows[0].data.get("o11"), Some(&Value::String("11th".to_string())));
    assert_eq!(r.rows[0].data.get("o12"), Some(&Value::String("12th".to_string())));
    assert_eq!(r.rows[0].data.get("o21"), Some(&Value::String("21st".to_string())));
}
...
fn test_si_prefix_kilo() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": 1500})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = SI_PREFIX(n) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("1.5k".to_string())));
}
...
fn test_si_prefix_mega() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": 1500000})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = SI_PREFIX(n) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("out"), Some(&Value::String("1.5M".to_string())));
}
...
fn test_parse_size_mb() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"sz": "1.5 MB"})).unwrap();
...
fn test_parse_size_kb() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"sz": "2 KB"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = PARSE_SIZE(sz) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_parse_duration_hours_minutes() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dur": "1h 30m"})).unwrap();
...
fn test_parse_duration_seconds_ms() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dur": "2s 500ms"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = PARSE_DURATION(dur) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("out") {
        Some(Value::Float(v)) => {
...
fn test_aliases_atof() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n": "2.718"})).unwrap();
