...
fn test_extended_gcd_basic() {
    ...
    match r.rows[0].data.get("res") {
        Some(Value::Object(m)) => {
            ...
            let x = match m.get("x") { Some(Value::Integer(v)) => *v, _ => panic!("no x") };
            let y = match m.get("y") { Some(Value::Integer(v)) => *v, _ => panic!("no y") };
...
fn test_crt_basic() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rs": [2, 3], "ms": [3, 5]})).unwrap();
...
fn test_crt_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rs": [0, 3, 4], "ms": [3, 4, 5]})).unwrap();
    // x ≡ 0 (mod 3), x ≡ 3 (mod 4), x ≡ 4 (mod 5) => x = 39
    let mut p = Parser::new(r#"QUERY t COMPUTE res = CRT(rs, ms) SELECT res;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let val = match r.rows[0].data.get("res") { Some(Value::Integer(v)) => *v, _ => panic!("not int") };
    assert_eq!(val % 3, 0);
    assert_eq!(val % 4, 3);
    assert_eq!(val % 5, 4);
}
...
fn test_lucas_num_sequence() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = LUCAS_SEQUENCE(2), b = LUCAS_SEQUENCE(3), c = LUCAS_SEQUENCE(4), d = LUCAS_SEQUENCE(5) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_motzkin_numbers() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = MOTZKIN(0), b = MOTZKIN(1), c = MOTZKIN(2), d = MOTZKIN(3), e = MOTZKIN(4) SELECT a, b, c, d, e;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_delannoy_base() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = DELANNOY(0, 0), b = DELANNOY(1, 0), c = DELANNOY(0, 1) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(1)));
...
fn test_delannoy_values() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = DELANNOY(1, 1), b = DELANNOY(2, 2), c = DELANNOY_NUMBER(2, 1) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(3)));
...
fn test_narayana_basic() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dummy": 1})).unwrap();
    // N(1,1)=1, N(4,2)=6
    let mut p = Parser::new(r#"QUERY t COMPUTE a = NARAYANA(1, 1), b = NARAYANA(4, 2) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
...
fn test_euler_numbers() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dummy": 1})).unwrap();
    // E(0)=1, E(2)=-1, E(4)=5
    let mut p = Parser::new(r#"QUERY t COMPUTE a = EULER_NUM(0), b = EULER_NUM(2), c = EULER_NUM(4), d = EULER_NUMBER(1) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(-1)));
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::Integer(5)));
...
fn test_bernoulli_numbers() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = BERNOULLI(0), b = BERNOULLI(1), c = BERNOULLI_NUMBER(2) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_tangent_numbers() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dummy": 1})).unwrap();
    // T(1)=1, T(3)=2, T(5)=16
    let mut p = Parser::new(r#"QUERY t COMPUTE a = TANGENT_NUMBER(1), b = ZAG_NUMBER(3), c = TANGENT_NUMBER(5), d = TANGENT_NUMBER(2) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
...
fn test_n_choose_k() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = N_CHOOSE_K(5, 2), b = N_CHOOSE_K(10, 3), c = N_CHOOSE_K(6, 0) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_permutations() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dummy": 1})).unwrap();
    // P(5,2)=20, P(4,4)=24
    let mut p = Parser::new(r#"QUERY t COMPUTE a = PERMUTATIONS(5, 2), b = N_PERM_K(4, 4) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_stirling_first() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = STIRLING_FIRST(4, 2), b = STIRLING1(5, 3), c = STIRLING_FIRST(3, 1) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_stirling_second() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = STIRLING_SECOND(4, 2), b = STIRLING2(5, 3), c = STIRLING_SECOND(3, 1) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_derangement() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = DERANGEMENT(0), b = DERANGEMENT(1), c = SUBFACTORIAL(2), d = DERANGEMENT(3), e = DERANGEMENT(4) SELECT a, b, c, d, e;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
...
fn test_set_partitions() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = SET_PARTITIONS(0), b = BELL_PARTITION(1), c = SET_PARTITIONS(2), d = SET_PARTITIONS(3), e = SET_PARTITIONS(4) SELECT a, b, c, d, e;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::Integer(2)));
...
fn test_integer_partitions() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = INTEGER_PARTITIONS(5, 2), b = PARTITION_INTO(5, 3), c = INTEGER_PARTITIONS(4, 2) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(2)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(2)));
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::Integer(2)));
...
fn test_sigma_function() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = SIGMA_FUNCTION(1), b = PERFECT_NUMBER_SIGMA(6), c = SIGMA_FUNCTION(12) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
...
fn test_liouville_function() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = LIOUVILLE_FUNCTION(1), b = LIOUVILLE(2), c = LIOUVILLE(4) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(-1)));
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::Integer(1)));
...
fn test_von_mangoldt() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = MANGOLDT_FUNCTION(1), b = VON_MANGOLDT(2), c = MANGOLDT_FUNCTION(4), d = VON_MANGOLDT(6) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_mobius_function() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = MOBIUS_FUNCTION(1), b = MOBIUS(2), c = MOBIUS(6), d = MOBIUS(4) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(-1)));
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::Integer(1)));
...
fn test_radical() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = RADICAL(12), b = RADICAL_N(1), c = RADICAL(8) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_sieve_primes_alias() {
    ...
    match r.rows[0].data.get("res") {
        Some(Value::Array(arr)) => {
            assert_eq!(arr, &vec![Value::Integer(2), Value::Integer(3), Value::Integer(5), Value::Integer(7)]);
        }
...
fn test_nth_prime() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = NTH_PRIME(1), b = NTH_PRIME(2), c = PRIME_N(3), d = NTH_PRIME(10) SELECT a, b, c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(2)));
...
fn test_prime_pi() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = PRIME_PI(10), b = PRIME_COUNT(100), c = PRIME_PI(1) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_prev_prime() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = PREV_PRIME(10), b = PREV_PRIME_BEFORE(8), c = PREV_PRIME(3) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(7)));
...
fn test_is_smooth() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = IS_SMOOTH(12, 3), b = B_SMOOTH(15, 3), c = IS_SMOOTH(1, 2) SELECT a, b, c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_smooth_numbers() {
    ...
    match r.rows[0].data.get("res") {
        Some(Value::Array(arr)) => {
            assert_eq!(arr, &vec![Value::Integer(1), Value::Integer(2), Value::Integer(4), Value::Integer(8), Value::Integer(16)]);
        }
...
fn test_find_smooth_alias() {
    ...
    match r.rows[0].data.get("res") {
        Some(Value::Array(arr)) => {
            ...
            let got: Vec<i64> = arr.iter().map(|v| match v { Value::Integer(i) => *i, _ => -1 }).collect();
...
fn test_sterling_second_boundary() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = STIRLING2(5, 1), b = STIRLING2(5, 5) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(1)));
}
...
fn test_integer_partitions_edge() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE a = INTEGER_PARTITIONS(7, 1), b = PARTITION_INTO(4, 4) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Integer(1)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Integer(1)));
}
