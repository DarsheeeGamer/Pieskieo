...
fn setup_scores() -> (tempfile::TempDir, Arc<PieskieoDb>, Executor) {
    ...
    for (name, score) in [("Alice", 90), ("Bob", 85), ("Carol", 92), ("Dave", 78), ("Eve", 88)] {
        db.put_doc_ns(None, Some("scores"), Uuid::new_v4(),
            serde_json::json!({"name": name, "score": score})).unwrap();
    }
    (dir, db, ex)
}
...
fn test_first_last_value() {
    ...
    for row in &r.rows {
        assert_eq!(row.data.get("first_s"), Some(&Value::Integer(78)),
            "FIRST_VALUE should be 78 for all rows");
    }

    // LAST_VALUE should be 92 (highest score)
    for row in &r.rows {
        assert_eq!(row.data.get("last_s"), Some(&Value::Integer(92)),
...
fn test_percent_rank() {
    ...
    match r.rows[0].data.get("pr") {
        Some(Value::Float(v)) => assert!((*v - 0.0).abs() < 1e-9, "First PERCENT_RANK should be 0.0, got {}", v),
    ...
    match r.rows[r.rows.len() - 1].data.get("pr") {
        Some(Value::Float(v)) => assert!((*v - 1.0).abs() < 1e-9, "Last PERCENT_RANK should be 1.0, got {}", v),
        other => panic!("Expected Float for PERCENT_RANK, got {:?}", other),
    }
}
...
fn test_nth_value() {
    ...
    for row in &r.rows {
        assert_eq!(row.data.get("nth"), Some(&Value::Integer(85)),
...
fn test_cume_dist() {
    ...
    match r.rows[r.rows.len() - 1].data.get("cd") {
        Some(Value::Float(v)) => assert!((*v - 1.0).abs() < 1e-9, "Last CUME_DIST should be 1.0, got {}", v),
    ...
    match r.rows[0].data.get("cd") {
        Some(Value::Float(v)) => assert!((*v - 0.2).abs() < 1e-9, "First CUME_DIST should be 0.2, got {}", v),
        other => panic!("Expected Float for CUME_DIST, got {:?}", other),
    }
}
