...
fn test_base64_url_decode_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"encoded": "aGVsbG8"}),
    )
    .unwrap();
...
fn test_base64_url_roundtrip() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE enc = BASE64_URL_ENCODE(msg) COMPUTE dec = BASE64_URL_DECODE(enc) SELECT dec;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
...
fn test_base32_encode_hello() {
    ...
    match result.rows[0].data.get("r") {
        Some(Value::String(s)) => {
            assert!(
                s.starts_with("NBSWY3DP"),
                "BASE32_ENCODE(hello) should start with NBSWY3DP, got {}",
                s
            );
        }
...
fn test_to_base32_alias() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("IE======".to_string()))
    );
}
...
fn test_base32_decode_hello() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"encoded": "NBSWY3DP"}),
    )
    .unwrap();
...
fn test_base32_roundtrip() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE enc = BASE32_ENCODE(msg) COMPUTE dec = BASE32_DECODE(enc) SELECT dec;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("dec"),
...
fn test_base16_encode_hello() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("6869".to_string()))
    );
}
...
fn test_base16_encode_uppercase() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("C3BF".to_string()))
    );
}
...
fn test_base58_encode_hello() {
    ...
    match result.rows[0].data.get("r") {
        Some(Value::String(s)) => {
            assert!(
                !s.is_empty(),
                "BASE58_ENCODE should return non-empty string"
            );
            // Verify it only contains base58 characters
            let b58_chars = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
            assert!(
                s.chars().all(|c| b58_chars.contains(c)),
                "Result should only contain base58 chars: {}",
                s
            );
        }
        other => panic!("Expected String, got {:?}", other),
...
fn test_base58_roundtrip() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE enc = BASE58_ENCODE(msg) COMPUTE dec = BASE58_DECODE(enc) SELECT dec;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("dec"),
...
fn test_from_base58_alias() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE enc = TO_BASE58(msg) COMPUTE dec = FROM_BASE58(enc) SELECT dec;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("dec"),
...
fn test_url_encode_component_safe_chars() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"msg": "abc-123_ok.~"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = URL_ENCODE_COMPONENT(msg) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("abc-123_ok.~".to_string()))
    );
}
...
fn test_xml_escape_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"msg": "<tag attr=\"val\">"}),
    )
    .unwrap();
...
fn test_is_base64_invalid() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"encoded": "not_base64!"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IS_BASE64(encoded) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_hex_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"h": "ABCDEF0123456789"}),
    )
    .unwrap();
...
fn test_is_ascii_false_for_utf8() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"msg": "caf\u{00E9}"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IS_ASCII(msg) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_char_code_roundtrip() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE code = CHAR_CODE(c) COMPUTE ch = CODE_TO_CHAR(code) SELECT ch;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("ch"),
...
fn test_str_to_bytes_alias() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::Array(vec![Value::Integer(65), Value::Integer(66),]))
    );
}
...
fn test_bytes_to_string_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"b": [72, 101, 108, 108, 111]}),
    )
    .unwrap();
...
fn test_string_bytes_roundtrip() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE bts = STRING_TO_BYTES(msg) COMPUTE back = BYTES_TO_STRING(bts) SELECT back;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("back"),
        Some(&Value::String("Hello".to_string()))
    );
}
...
fn test_rle_encode_array_string() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("3A3B2C".to_string()))
    );
}
...
fn test_rle_arr_alias() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("2AB2C".to_string()))
    );
}
...
fn test_rle_decode_string_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"encoded": "3A3B2C"}),
    )
    .unwrap();
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("AAABBBCC".to_string()))
    );
}
...
fn test_rld_str_alias() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("AABCC".to_string()))
    );
}
...
fn test_rle_string_roundtrip() {
    ...
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE enc = RLE_ENCODE_ARRAY(msg) COMPUTE dec = RLE_DECODE_STRING(enc) SELECT dec;"#,
    );
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("dec"),
...
fn test_compress_ratio_repeated_chars() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"msg": "AAAAAAAAAA"}),
    )
    .unwrap();
    ...
    match result.rows[0].data.get("r") {
        Some(Value::Float(f)) => {
            assert!(
                *f < 1.0,
                "Highly repeated string should have ratio < 1.0, got {}",
                f
            );
        }
...
fn test_detect_encoding_numeric() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("numeric".to_string()))
    );
}
...
fn test_detect_encoding_hex() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::String("hex".to_string()))
    );
}
...
fn test_is_json_string_object() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"payload": "{\"key\":\"val\"}"}),
    )
    .unwrap();
...
fn test_is_csv_row_false() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"line": "hello world"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IS_CSV_ROW(line) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_csv_quote_field_basic() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"fld": "hello world"}),
    )
    .unwrap();
