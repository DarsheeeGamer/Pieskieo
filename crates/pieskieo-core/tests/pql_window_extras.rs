...
fn test_harm_mean_alias() {
    ...
    let (_dir, _db, ex) = setup_db_with_data(
        "d",
        vec![
            serde_json::json!({"v": 1}),
            serde_json::json!({"v": 2}),
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE hm = HARM_MEAN(v) SELECT hm;");
...
fn test_trimmed_mean() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE g = 1 GROUP BY g COMPUTE tm = TRIMMED_MEAN(v, 0.2) SELECT tm;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("tm") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - 3.0).abs() < 0.001,
                "TRIMMED_MEAN should be 3.0, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_coefficient_of_variation() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE g = 1 GROUP BY g COMPUTE cv = COEFFICIENT_OF_VARIATION(v) SELECT cv;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("cv") {
        Some(Value::Float(f)) => {
            assert!(
                *f > 0.0,
                "COEFFICIENT_OF_VARIATION should be positive, got {}",
                f
            );
...
fn test_mean_abs_dev_alias() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE g = 1 GROUP BY g COMPUTE mad_val = MEAN_ABS_DEV(v) SELECT mad_val;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_median_absolute_deviation() {
    ...
    match r.rows[0].data.get("mad_med") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - 1.0).abs() < 0.001,
                "MEDIAN_ABSOLUTE_DEVIATION should be 1.0, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_spearman_correlation() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE g = 1 GROUP BY g COMPUTE sc = SPEARMAN_CORRELATION(x, y) SELECT sc;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("sc") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - 1.0).abs() < 0.001,
                "SPEARMAN_CORRELATION of perfectly correlated data should be 1.0, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_spearman_r_alias() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE sc = SPEARMAN_R(x, y) SELECT sc;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("sc") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - (-1.0)).abs() < 0.001,
                "SPEARMAN_R of anti-correlated data should be -1.0, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_nth_value_agg() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE gk = g GROUP BY gk COMPUTE nth = NTH_VALUE_AGG(v, 2) SELECT nth;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_group_concat() {
    ...
    match r.rows[0].data.get("names") {
        Some(Value::String(s)) => {
            ...
            assert!(
                s.contains("Carol"),
                "Result should contain Carol, got: {}",
                s
            );
        }
...
fn test_bit_and_agg() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE band = BIT_AND_AGG(v) SELECT band;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_bit_or_agg() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE bor = BIT_OR_AGG(v) SELECT bor;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_bit_xor_agg() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE bxor = BIT_XOR_AGG(v) SELECT bxor;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_collect_set() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE s = COLLECT_SET(v) SELECT s;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("s") {
        Some(Value::Array(arr)) => {
            assert_eq!(
                arr.len(),
                3,
                "COLLECT_SET should have 3 distinct values, got {}",
                arr.len()
            );
        }
...
fn test_collect_list() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE lst = COLLECT_LIST(v) SELECT lst;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("lst") {
        Some(Value::Array(arr)) => {
            assert_eq!(
                arr.len(),
                4,
                "COLLECT_LIST should have 4 values (with duplicates), got {}",
                arr.len()
            );
        }
        other => panic!("Expected Array, got {:?}", other),
...
fn test_median_ad_alias() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE mad_v = MEDIAN_AD(v) SELECT mad_v;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
...
fn test_pearson_r_alias() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE pr = PEARSON_R(x, y) SELECT pr;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("pr") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - 1.0).abs() < 0.001,
                "PEARSON_R should be 1.0 for perfect correlation, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_rank_corr_alias() {
    ...
    let mut p =
        Parser::new("QUERY d COMPUTE g = 1 GROUP BY g COMPUTE rc = RANK_CORR(x, y) SELECT rc;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("rc") {
        Some(Value::Float(f)) => {
            assert!(
                (*f - (-1.0)).abs() < 0.001,
                "RANK_CORR should be -1.0 for anti-correlated data, got {}",
                f
            );
        }
        other => panic!("Expected Float, got {:?}", other),
...
fn test_listagg_alias() {
    ...
    let mut p = Parser::new(
        "QUERY d COMPUTE g = 1 GROUP BY g COMPUTE agg = LISTAGG(name, \"|\") SELECT agg;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty(), "Expected at least one result row");
    match r.rows[0].data.get("agg") {
        Some(Value::String(s)) => {
            assert!(
                s.contains('|'),
                "LISTAGG result should contain separator '|', got: {}",
                s
            );
            assert!(
                s.contains('X'),
                "LISTAGG result should contain 'X', got: {}",
                s
            );
        }
        other => panic!("Expected String, got {:?}", other),
