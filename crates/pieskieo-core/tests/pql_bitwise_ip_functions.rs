...
fn test_popcount() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"a": 255, "b": 0, "c": 7}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE pa = POPCOUNT(a) COMPUTE pb = POPCOUNT(b) COMPUTE pc = POPCOUNT(c) SELECT pa, pb, pc;"#,
    );
...
fn test_bit_get_set_clear() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"n": 5, "pos0": 0, "pos1": 1}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE b0 = BIT_GET(n, pos0) COMPUTE b1 = BIT_GET(n, pos1) COMPUTE set1 = BIT_SET(n, pos1) COMPUTE clr0 = BIT_CLEAR(n, pos0) SELECT b0, b1, set1, clr0;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_ipv4_ipv6() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"v4": "10.0.0.1", "v6": "::1", "bad": "hello"}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE a = IS_IPV4(v4) COMPUTE b = IS_IPV4(bad) COMPUTE c = IS_IPV6(v6) SELECT a, b, c;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_ip_in_cidr() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"ip1": "192.168.1.100", "ip2": "10.0.0.1", "cidr": "192.168.1.0/24"}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE in_net = IP_IN_CIDR(ip1, cidr) COMPUTE out_net = IP_IN_CIDR(ip2, cidr) SELECT in_net, out_net;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_private_ip() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"private": "192.168.1.1", "public": "8.8.8.8", "rfc172": "172.16.5.1"}),
    )
    .unwrap();

    let mut p = Parser::new(
        r#"QUERY t COMPUTE a = IS_PRIVATE_IP(private) COMPUTE b = IS_PRIVATE_IP(public) COMPUTE c = IS_PRIVATE_IP(rfc172) SELECT a, b, c;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Bool(true)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Bool(false)));
