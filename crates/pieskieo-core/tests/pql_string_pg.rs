...
fn test_concat_ws_basic() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"a": "foo", "b": "bar", "c": "baz"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = CONCAT_WS(", ", a, b, c) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("foo, bar, baz".to_string())));
}
...
fn test_concat_ws_skips_null() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"a": "foo", "c": "baz"})).unwrap();
    // b is missing (null), should be skipped
    let mut p = Parser::new(r#"QUERY t COMPUTE r = CONCAT_WS("-", a, b, c) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("foo-baz".to_string())));
}
...
fn test_overlay_basic() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("hello there".to_string())));
}
...
fn test_overlay_no_length() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("XYllo world".to_string())));
}
...
fn test_convert_passthrough() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("hello".to_string())));
}
...
fn test_convert_from_passthrough() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "world"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = CONVERT_FROM(s, "UTF8") SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("world".to_string())));
}
...
fn test_convert_to_passthrough() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "data"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = CONVERT_TO(s, "UTF8") SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("data".to_string())));
}
...
fn test_encode_hex() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("4142".to_string())));
}
...
fn test_encode_base64() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "Man"})).unwrap();
    // base64("Man") = "TWFu"
    let mut p = Parser::new(r#"QUERY t COMPUTE r = ENCODE(s, "base64") SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("TWFu".to_string())));
}
...
fn test_regexp_split_to_table() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "one1two2three"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE parts = REGEXP_SPLIT_TO_TABLE(s, "[0-9]") SELECT parts;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_substring_index_positive() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("a.b".to_string())));
}
...
fn test_substring_index_negative() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE r = SUBSTRING_INDEX(s, ".", -2) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("c.d".to_string())));
}
...
fn test_parse_ident_dotted() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "public.my_table"})).unwrap();
...
fn test_parse_ident_quoted() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": r#""schema"."table""#})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = PARSE_IDENT(s) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("r") {
        Some(Value::Array(arr)) => {
...
fn test_format_type_known_oids() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ti = FORMAT_TYPE(23, 0) COMPUTE tt = FORMAT_TYPE(25, 0) SELECT ti, tt;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ti"), Some(&Value::String("integer".to_string())));
    assert_eq!(r.rows[0].data.get("tt"), Some(&Value::String("text".to_string())));
}
...
fn test_format_type_unknown_oid() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("type_99999".to_string())));
}
...
fn test_concat_ws_with_encode() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"a": "foo", "b": "bar"})).unwrap();
    // Build a joined string, then encode it in hex
    let mut p = Parser::new(r#"QUERY t COMPUTE joined = CONCAT_WS(":", a, b) COMPUTE enc = ENCODE(joined, "hex") SELECT enc;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    // "foo:bar" in hex = 666f6f3a626172
    assert_eq!(r.rows[0].data.get("enc"), Some(&Value::String("666f6f3a626172".to_string())));
}
...
fn test_overlay_with_substring_index() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"filepath": "/home/user/file.txt"})).unwrap();
    // Get first two path segments then get octet length
    let mut p = Parser::new(r#"QUERY t COMPUTE first2 = SUBSTRING_INDEX(filepath, "/", 3) COMPUTE r = OCTET_LENGTH(first2) SELECT first2, r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("first2"), Some(&Value::String("/home/user".to_string())));
...
fn test_regexp_split_to_table_single_result() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "nodivider"})).unwrap();
...
fn test_parse_ident_single() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "mytable"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = PARSE_IDENT(s) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("r") {
        Some(Value::Array(arr)) => {
...
fn test_to_ascii_various_codepoints() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"n1": 97, "n2": 90, "n3": 48})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE a = TO_ASCII(n1) COMPUTE z = TO_ASCII(n2) COMPUTE zero = TO_ASCII(n3) SELECT a, z, zero;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::String("a".to_string())));
    assert_eq!(r.rows[0].data.get("z"), Some(&Value::String("Z".to_string())));
    assert_eq!(r.rows[0].data.get("zero"), Some(&Value::String("0".to_string())));
}
...
fn test_encode_base64_padding() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"s": "M"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = ENCODE(s, "base64") SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("TQ==".to_string())));
}
...
fn test_overlay_start_at_end() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("r"), Some(&Value::String("abcXYZ".to_string())));
}
...
fn test_format_type_boolean_uuid() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE b = FORMAT_TYPE(16, 0) COMPUTE u = FORMAT_TYPE(2950, 0) SELECT b, u;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::String("boolean".to_string())));
    assert_eq!(r.rows[0].data.get("u"), Some(&Value::String("uuid".to_string())));
}
