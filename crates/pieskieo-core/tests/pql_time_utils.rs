...
fn test_seconds_to_hms_3661() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 3661})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = SECONDS_TO_HMS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("01:01:01".to_string())));
}
...
fn test_seconds_to_hms_zero() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 0})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = SECONDS_TO_HMS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("00:00:00".to_string())));
}
...
fn test_seconds_to_hms_86400() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 86400})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = SECONDS_TO_HMS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("24:00:00".to_string())));
}
...
fn test_sec_to_hms_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 7322})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = SEC_TO_HMS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("02:02:02".to_string())));
}
...
fn test_hms_to_seconds_010101() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "01:01:01"})).unwrap();
...
fn test_hms_to_seconds_zero() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "00:00:00"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = HMS_TO_SECONDS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_hms_to_sec_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "02:30:00"})).unwrap();
...
fn test_seconds_to_hms_round_trip() {
    ...
    let (db1, ex1) = setup();
    db1.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 7384})).unwrap();
    ...
    let (db2, ex2) = setup();
    db2.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": hms})).unwrap();
...
fn test_seconds_to_parts_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 3600})).unwrap();
...
fn test_duration_parts_days() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 90061})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = DURATION_PARTS(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    if let Some(Value::Object(obj)) = r.rows[0].data.get("nv") {
...
fn test_time_add_seconds_plus_one_hour() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "01:00:00", "nv": 3600})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE tv = TIME_ADD_SECONDS(t1, nv) SELECT tv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("tv"), Some(&Value::String("02:00:00".to_string())));
}
...
fn test_time_add_seconds_overflow_hour() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "23:00:00", "nv": 7200})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE tv = TIME_ADD_SECONDS(t1, nv) SELECT tv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("tv"), Some(&Value::String("25:00:00".to_string())));
}
...
fn test_add_seconds_to_time_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "00:00:00", "nv": 90})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE tv = ADD_SECONDS_TO_TIME(t1, nv) SELECT tv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("tv"), Some(&Value::String("00:01:30".to_string())));
}
...
fn test_time_diff_seconds_positive() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "01:00:00", "t2": "02:00:00"})).unwrap();
...
fn test_time_diff_seconds_negative() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "02:00:00", "t2": "01:00:00"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = TIME_DIFF_SECONDS(t1, t2) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Integer(-3600)));
}
...
fn test_time_subtract_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "00:30:00", "t2": "01:00:00"})).unwrap();
...
fn test_is_valid_time_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "12:30:45"})).unwrap();
...
fn test_is_valid_time_invalid_minutes() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "12:60:00"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_VALID_TIME(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_valid_time_invalid_seconds() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "12:30:60"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_VALID_TIME(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Bool(false)));
}
...
fn test_is_time_string_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "25:00:00"})).unwrap();
...
fn test_epoch_to_date_zero() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("1970-01-01".to_string())));
}
...
fn test_epoch_to_date_one_day() {
    ...
    let mut p = Parser::new("QUERY t COMPUTE nv = EPOCH_TO_DATE(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("1970-01-02".to_string())));
}
...
fn test_epoch_to_date_2021_01_01() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 1609459200i64})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = EPOCH_TO_DATE(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2021-01-01".to_string())));
}
...
fn test_date_to_epoch_1970_01_01() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "1970-01-01"})).unwrap();
...
fn test_date_to_epoch_1970_01_02() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": "1970-01-02"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = DATE_TO_EPOCH(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_epoch_to_date_and_back_round_trip() {
    ...
    let (db2, ex2) = setup();
    db2.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": date_str})).unwrap();
...
fn test_datetime_diff_seconds_one_day() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"d1": "2024-01-01", "d2": "2024-01-02"})).unwrap();
...
fn test_is_valid_time_not_string() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tv": 12345})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_VALID_TIME(tv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Bool(false)));
}
...
fn test_time_diff_seconds_same_time() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "05:30:00", "t2": "05:30:00"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = TIME_DIFF_SECONDS(t1, t2) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Integer(0)));
}
