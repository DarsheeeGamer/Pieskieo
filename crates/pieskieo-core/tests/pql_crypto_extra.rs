...
fn test_crc32_hash_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = CRC32(sv) COMPUTE b = CRC32_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_crc16_hash_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"sv": "world"}),
    )
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = CRC16(sv) COMPUTE b = CRC16_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
...
fn test_adler_hash_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = ADLER32(sv) COMPUTE b = ADLER_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "ADLER32 and ADLER_HASH must produce identical results"
    );
}
...
fn test_fnv1a_hash_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = FNV1A(sv) COMPUTE b = FNV1A_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
...
fn test_djb2_hash_alias_equals_djbx33a() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = DJBX33A(sv) COMPUTE b = DJB2_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
...
fn test_djbx33a_empty_vs_hello_differ() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"sv": "", "sv2": "hello"}),
    )
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = DJBX33A(sv) COMPUTE b = DJBX33A(sv2) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
...
fn test_murmur3_hash_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = MURMUR3(sv) COMPUTE b = MURMUR3_HASH(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "MURMUR3 and MURMUR3_HASH must produce identical results"
    );
}
...
fn test_jenkins_oat_alias() {
    ...
    let mut p = Parser::new(
        "QUERY t COMPUTE a = JENKINS_HASH(sv) COMPUTE b = JENKINS_OAT(sv) SELECT a, b;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "JENKINS_HASH and JENKINS_OAT must produce identical results"
    );
}
...
fn test_poly_hash_alias() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE a = POLYNOMIAL_HASH(sv) COMPUTE b = POLY_HASH(sv) SELECT a, b;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "POLYNOMIAL_HASH and POLY_HASH must produce identical results"
    );
}
...
fn test_polynomial_hash_explicit_equals_default() {
    ...
    assert_eq!(
        a, b,
        "POLYNOMIAL_HASH default args must match explicit args"
    );
}
...
fn test_is_valid_isbn_alias() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE a = ISBN_CHECK(sv) COMPUTE b = IS_VALID_ISBN(sv) SELECT a, b;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "ISBN_CHECK and IS_VALID_ISBN must produce identical results"
    );
}
...
fn test_isbn_check_valid_isbn13() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"sv": "9780306406157"}),
    )
    .unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = ISBN_CHECK(sv) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_verhoeff_check_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new("QUERY t COMPUTE a = VERHOEFF(sv) COMPUTE b = VERHOEFF_CHECK(sv) SELECT a, b;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let a = r.rows[0].data.get("a").cloned();
    let b = r.rows[0].data.get("b").cloned();
    assert_eq!(
        a, b,
        "VERHOEFF and VERHOEFF_CHECK must produce identical results"
    );
}
...
fn test_hash_combine_returns_integer() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"nv": 1234, "nv2": 5678}),
    )
    .unwrap();
...
fn test_combine_hashes_alias() {
    ...
    assert_eq!(
        a, b,
        "HASH_COMBINE and COMBINE_HASHES must produce identical results"
    );
}
...
fn test_crc32_different_strings_differ() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"sv": "foo", "sv2": "bar"}),
    )
    .unwrap();
