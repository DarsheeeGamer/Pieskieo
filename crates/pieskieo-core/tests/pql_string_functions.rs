...
fn test_str_split_and_split_part() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE parts = STR_SPLIT(s, ",") COMPUTE second = SPLIT_PART(s, ",", 2) SELECT parts, second;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_array_to_string() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"tags": ["rust", "db", "fast"]}),
    )
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE joined = ARRAY_TO_STRING(tags, ", ") SELECT joined;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("joined"),
        Some(&Value::String("rust, db, fast".to_string()))
    );
}
...
fn test_slugify() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"title": "Hello World! This is a Test"}),
    )
    .unwrap();
    let mut p = Parser::new("QUERY t COMPUTE slug = SLUGIFY(title) SELECT slug;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("slug"),
        Some(&Value::String("hello-world-this-is-a-test".to_string()))
    );
}
