...
fn test_is_valid_ipv4() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "192.168.1.1", "bad": "999.0.0.1"}),
    )
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE ok = IS_VALID_IPV4(addr) COMPUTE bad = IS_VALID_IPV4(bad) SELECT ok, bad;"#,
    );
...
fn test_is_valid_ipv4_invalid_octet() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "256.1.1.1"}),
    )
    .unwrap();
...
fn test_is_valid_ipv6() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "2001:0db8:0000:0000:0000:0000:0000:0001"}),
    )
    .unwrap();
...
fn test_is_ipv6_compressed() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "2001:db8::1"}),
    )
    .unwrap();
...
fn test_is_ip_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "not-an-ip"}),
    )
    .unwrap();
...
fn test_ip_octets() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "192.168.1.5"}),
    )
    .unwrap();
...
fn test_ip_addr_version_v4() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "192.168.0.1"}),
    )
    .unwrap();
...
fn test_ip_in_subnet_true() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "192.168.1.100", "net": "192.168.1.0/24"}),
    )
    .unwrap();
...
fn test_ip_in_subnet_false() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "10.0.0.1", "net": "192.168.1.0/24"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IP_IN_SUBNET(addr, net) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_cidr_includes_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "10.1.2.3", "net": "10.0.0.0/8"}),
    )
    .unwrap();
...
fn test_subnet_network() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "192.168.1.0/24"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("192.168.1.0".to_string()))
    );
    drop(dir);
}
...
fn test_subnet_network_slash16() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "172.16.5.42/16"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = SUBNET_NETWORK(cidr) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("172.16.0.0".to_string()))
    );
    drop(dir);
}
...
fn test_subnet_broadcast() {
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("192.168.1.255".to_string()))
    );
    drop(dir);
}
...
fn test_cidr_mask_24() {
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("255.255.255.0".to_string()))
    );
    drop(dir);
}
...
fn test_subnet_mask_alias_16() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"cidr": "172.16.0.0/16"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("255.255.0.0".to_string()))
    );
    drop(dir);
}
...
fn test_cidr_mask_8() {
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("255.0.0.0".to_string()))
    );
    drop(dir);
}
...
fn test_is_loopback_ip_false() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "192.168.1.1"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IS_LOOPBACK_IP(addr) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_multicast_true() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "224.0.0.1"}),
    )
    .unwrap();
...
fn test_is_link_local_true() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "169.254.1.1"}),
    )
    .unwrap();
...
fn test_is_apipa_alias_true() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "169.254.0.1"}),
    )
    .unwrap();
...
fn test_ipv4_class_b() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "172.16.0.1"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IPV4_CLASS(addr) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_ipv4_class_e() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"addr": "240.0.0.1"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IPV4_CLASS(addr) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_url_domain() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "https://www.example.com/page"}),
    )
    .unwrap();
...
fn test_url_domain_strips_port() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "http://example.com:8080/api"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = URL_DOMAIN(url) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_extract_path() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "https://example.com/foo/bar?q=1"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("/foo/bar".to_string()))
    );
    drop(dir);
}
...
fn test_extract_query_string() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "https://example.com?foo=1&bar=2"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("foo=1&bar=2".to_string()))
    );
    drop(dir);
}
...
fn test_hostname_from_url() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "https://api.example.com/v1/foo"}),
    )
    .unwrap();
...
fn test_hostname_from_url_strips_port() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "http://localhost:3000/app"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = HOSTNAME_FROM_URL(url) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_port_from_url() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "http://example.com:8080/path"}),
    )
    .unwrap();
...
fn test_url_port_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"url": "https://example.com:443/"}),
    )
    .unwrap();
...
fn test_is_valid_mac_invalid() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"mac": "AA:BB:CC:DD:EE"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = IS_VALID_MAC(mac) SELECT r;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_mac_to_int() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"mac": "00:00:00:00:00:01"}),
    )
    .unwrap();
...
fn test_mac_addr_to_int_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"mac": "00:00:00:00:01:00"}),
    )
    .unwrap();
...
fn test_normalize_mac_from_lowercase() {
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("AA:BB:CC:DD:EE:FF".to_string()))
    );
    drop(dir);
}
...
fn test_mac_oui() {
    ...
    assert_eq!(
        r.rows[0].data.get("r"),
        Some(&Value::String("AA:BB:CC".to_string()))
    );
    drop(dir);
}
