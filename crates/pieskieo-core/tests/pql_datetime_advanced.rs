...
fn test_iso_week() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-15"})).unwrap();
...
fn test_iso_year() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-01"})).unwrap();
...
fn test_iso_year_week_year_alias() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2020-12-31"})).unwrap();
...
fn test_start_of_month() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-03-15"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE s = START_OF_MONTH(dt) SELECT s;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("s"), Some(&Value::String("2024-03-01".to_string())));
}
...
fn test_end_of_month() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("e"), Some(&Value::String("2024-03-31".to_string())));
}
...
fn test_end_of_month_february_leap() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-02-10"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE e = LAST_DAY(dt) SELECT e;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("e"), Some(&Value::String("2024-02-29".to_string())));
}
...
fn test_end_of_month_december() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-12-01"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE e = MONTH_END(dt) SELECT e;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("e"), Some(&Value::String("2024-12-31".to_string())));
}
...
fn test_start_of_year() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-06-15"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE s = START_OF_YEAR(dt) SELECT s;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("s"), Some(&Value::String("2024-01-01".to_string())));
}
...
fn test_age_in_years() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "1990-05-15", "dt_to": "2024-01-01"})).unwrap();
...
fn test_years_between_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2000-01-01", "dt_to": "2024-01-01"})).unwrap();
...
fn test_age_in_months() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-03-15"})).unwrap();
...
fn test_months_between_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-15", "dt_to": "2024-03-15"})).unwrap();
...
fn test_age_in_days() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-01-08"})).unwrap();
...
fn test_date_diff_days_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-02-01"})).unwrap();
...
fn test_date_diff_weeks() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-01-22"})).unwrap();
...
fn test_weeks_between_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-01-29"})).unwrap();
...
fn test_date_diff_business_days() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-01-08"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE b = DATE_DIFF_BUSINESS_DAYS(dt_from, dt_to) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_business_days_between_alias() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt_from": "2024-01-01", "dt_to": "2024-01-05"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE b = BUSINESS_DAYS_BETWEEN(dt_from, dt_to) SELECT b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_next_weekday_from_friday() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-05"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE n = NEXT_WEEKDAY(dt) SELECT n;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("n"), Some(&Value::String("2024-01-08".to_string())));
}
...
fn test_next_weekday_from_saturday() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-06"})).unwrap();
...
fn test_prev_weekday() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-08"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE p = PREV_WEEKDAY(dt) SELECT p;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("p"), Some(&Value::String("2024-01-05".to_string())));
}
...
fn test_prev_weekday_from_sunday() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-07"})).unwrap();
...
fn test_nth_weekday_in_month() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("d"), Some(&Value::String("2024-01-15".to_string())));
}
...
fn test_nth_weekday_in_month_friday() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("d"), Some(&Value::String("2024-01-05".to_string())));
}
...
fn test_fiscal_quarter_standard() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-04-01"})).unwrap();
...
fn test_make_interval_basic() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("iv"), Some(&Value::String("P1Y2M3D".to_string())));
}
...
fn test_make_interval_with_time() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("iv"), Some(&Value::String("P1DT2H30M".to_string())));
}
...
fn test_make_interval_zeros() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("iv"), Some(&Value::String("P0D".to_string())));
}
...
fn test_make_interval_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("iv"), Some(&Value::String("PT4H".to_string())));
}
