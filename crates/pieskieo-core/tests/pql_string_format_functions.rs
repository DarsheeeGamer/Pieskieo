...
fn test_camel_to_snake() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "helloWorldFoo"}),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some(&Value::String("hello_world_foo".to_string()))
    );
}
...
fn test_snake_to_camel() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "hello_world_foo"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("helloWorldFoo".to_string()))
    );
}
...
fn test_to_pascal_case_with_spaces() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "hello world foo"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = TO_PASCAL_CASE(s) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_palindrome_case_insensitive() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "A man a plan a canal Panama"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = IS_PALINDROME(s) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_squeeze() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "heeellllo  world"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("helo world".to_string()))
    );
}
...
fn test_interleave() {
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("a1b2c3".to_string()))
    );
}
...
fn test_interleave_unequal_length() {
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("a1b2cde".to_string()))
    );
}
...
fn test_pad_center() {
    ...
    match r.rows[0].data.get("out") {
        Some(Value::String(s)) => {
            ...
            assert!(
                s.trim() == "hi",
                "Expected only 'hi' after trim, got {:?}",
                s.trim()
            );
        }
...
fn test_wrap_text() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "hello world foo bar"}),
    )
    .unwrap();
...
fn test_expand_tabs() {
    ...
    match r.rows[0].data.get("out") {
        Some(Value::String(s)) => {
            ...
            assert!(
                s.starts_with('a'),
                "Expected result to start with 'a': {:?}",
                s
            );
...
fn test_normalize_whitespace() {
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("hello world foo".to_string()))
    );
}
...
fn test_title_case_smart() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "the lord of the rings"}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("The Lord of the Rings".to_string()))
    );
}
...
fn test_title_case_smart_first_word_always_capitalized() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "a tale of two cities"}),
    )
    .unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE out = TITLE_CASE_SMART(s) SELECT out;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("A Tale of Two Cities".to_string()))
    );
}
...
fn test_collapse_spaces_alias() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "  leading and  trailing  "}),
    )
    .unwrap();
    ...
    assert_eq!(
        r.rows[0].data.get("out"),
        Some(&Value::String("leading and trailing".to_string()))
    );
}
