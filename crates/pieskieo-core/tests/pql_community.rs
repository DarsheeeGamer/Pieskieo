...
fn make_square_with_diagonal() -> (Arc<PieskieoDb>, Executor, Uuid, Uuid, Uuid, Uuid) {
    ...
    db.put_doc_ns(
        ...
        Some("sq"),
        id4,
        serde_json::json!({"nid": id4.to_string()}),
    )
    .unwrap();
    // Ring edges
...
fn test_label_propagation_isolated_node() {
    ...
    .unwrap();
    let query = format!(r#"QUERY solo COMPUTE lbl = LABEL_PROPAGATION(nid, "solo") SELECT lbl;"#);
    let mut p = Parser::new(&query);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("lbl") {
...
fn test_connected_component_same_component() {
    ...
    db.put_doc_ns(
        ...
        Some("cc"),
        a,
        serde_json::json!({"nid": a.to_string()}),
    )
    .unwrap();
    db.put_doc_ns(
        None,
        Some("cc"),
        b,
        serde_json::json!({"nid": b.to_string()}),
    )
    .unwrap();
    ...
    assert_eq!(
        cid_a, cid_b,
        "connected nodes must share the same component ID"
    );
}
...
fn test_connected_component_different_components() {
    ...
    assert_ne!(
        cid_a, cid_b,
        "disconnected nodes must have different component IDs"
    );
}
...
fn test_union_find_component_alias() {
    ...
    .unwrap();
    let query = format!(r#"QUERY ufc COMPUTE cid = UNION_FIND_COMPONENT(nid, "ufc") SELECT cid;"#);
    let mut p = Parser::new(&query);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty());
...
fn test_graph_diameter_approx_path_graph() {
    ...
    let c = Uuid::new_v4();
    let d = Uuid::new_v4();
    db.put_doc_ns(None, Some("pathgraph"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("pathgraph"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.put_doc_ns(None, Some("pathgraph"), c, serde_json::json!({"name": "C"}))
        .unwrap();
    db.put_doc_ns(None, Some("pathgraph"), d, serde_json::json!({"name": "D"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    ...
    let mut p = Parser::new(
        r#"QUERY pathgraph COMPUTE g=1 GROUP BY g COMPUTE diam = GRAPH_DIAMETER_APPROX("pathgraph", 4) SELECT diam;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("diam") {
        Some(Value::Integer(n)) => assert!(
            *n >= 3,
            "pathgraph A-B-C-D diameter should be >= 3, got {}",
            n
        ),
        other => panic!("expected Integer, got {:?}", other),
    }
...
fn test_approx_diameter_alias() {
    ...
    let a = Uuid::new_v4();
    let b = Uuid::new_v4();
    db.put_doc_ns(None, Some("ad"), a, serde_json::json!({"name": "A"}))
        .unwrap();
    db.put_doc_ns(None, Some("ad"), b, serde_json::json!({"name": "B"}))
        .unwrap();
    db.add_edge(a, b, 1.0).unwrap();

    let mut p = Parser::new(
        r#"QUERY ad COMPUTE g=1 GROUP BY g COMPUTE diam = APPROX_DIAMETER("ad", 10) SELECT diam;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("diam") {
        Some(Value::Integer(n)) => {
            assert!(*n >= 1, "2-node graph diameter should be >= 1, got {}", n)
        }
        other => panic!("expected Integer, got {:?}", other),
    }
}
...
fn test_hub_score_hub_node() {
    ...
    let n2 = Uuid::new_v4();
    let n3 = Uuid::new_v4();
    db.put_doc_ns(None, Some("hs"), hub, serde_json::json!({"role": "hub"}))
        .unwrap();
    db.put_doc_ns(None, Some("hs"), n1, serde_json::json!({"role": "leaf"}))
        .unwrap();
    db.put_doc_ns(None, Some("hs"), n2, serde_json::json!({"role": "leaf"}))
        .unwrap();
    db.put_doc_ns(None, Some("hs"), n3, serde_json::json!({"role": "leaf"}))
        .unwrap();
    ...
    let query =
        format!(r#"QUERY hs WHERE role = "hub" COMPUTE h = HUB_SCORE(nid, "hs") SELECT h;"#);
    let mut p = Parser::new(&query);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert!(!r.rows.is_empty());
    match r.rows[0].data.get("h") {
        Some(Value::Float(f)) => assert!(
            *f >= 0.0 && *f <= 1.0,
            "hub score should be in [0,1], got {}",
            f
        ),
        other => panic!("expected Float, got {:?}", other),
    }
...
fn test_authority_score_authority_node() {
    ...
    match r.rows[0].data.get("a") {
        Some(Value::Float(f)) => assert!(
            *f >= 0.0 && *f <= 1.0,
            "authority score should be in [0,1], got {}",
            f
        ),
        other => panic!("expected Float, got {:?}", other),
    }
...
fn test_graph_overlap_common_neighbors() {
    ...
    db.put_doc_ns(
        ...
        Some("ov"),
        c,
        serde_json::json!({"nid": c.to_string()}),
    )
    .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(b, c, 1.0).unwrap();
...
fn test_graph_overlap_no_common_neighbors() {
    ...
    db.put_doc_ns(
        ...
        Some("noov"),
        d,
        serde_json::json!({"nid": d.to_string()}),
    )
    .unwrap();
    db.add_edge(a, b, 1.0).unwrap();
    db.add_edge(c, d, 1.0).unwrap();
...
fn test_preferential_attachment_score() {
    ...
    match r.rows[0].data.get("pa") {
        Some(Value::Integer(n)) => assert!(
            *n >= 4,
            "PA score for degree-2 nodes should be >= 4, got {}",
            n
        ),
        other => panic!("expected Integer, got {:?}", other),
    }
