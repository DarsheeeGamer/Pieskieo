...
fn test_npv_simple_cashflows() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rate": 0.1})).unwrap();
...
fn test_internal_rate_return_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE r = INTERNAL_RATE_RETURN([-1000.0, 400.0, 400.0, 400.0]) SELECT r;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("r") {
        Some(Value::Float(f)) => assert!(*f > 0.09 && *f < 0.11, "expected ~9.7% IRR, got {}", f),
        other => panic!("expected Float, got {:?}", other),
...
fn test_mirr_returns_float() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE m = MIRR([-1000.0, 300.0, 400.0, 500.0], 0.1, 0.12) SELECT m;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "MIRR should return a finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_modified_irr_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE m = MODIFIED_IRR([-1000.0, 300.0, 400.0, 500.0], 0.1, 0.12) SELECT m;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "MODIFIED_IRR should return a finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_cagr_compound_growth_rate_alias() {
    ...
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE c = COMPOUND_GROWTH_RATE(100.0, 200.0, 5.0) SELECT c;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("c") {
        Some(Value::Float(f)) => assert!((*f - 0.1487).abs() < 0.001, "expected ~14.87% CAGR, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_sharpe_positive_ratio() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE s = SHARPE([0.05, 0.06, 0.07, 0.08, 0.09], 0.0) SELECT s;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f > 0.0, "Sharpe ratio should be positive for positive returns, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_sharpe_ratio_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE s = SHARPE_RATIO([0.05, 0.06, 0.07, 0.08, 0.09], 0.0) SELECT s;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f > 0.0, "SHARPE_RATIO alias should give positive result, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_sortino_positive_with_positive_mean() {
    ...
    let mut p = Parser::new("QUERY t COMPUTE s = SORTINO([0.05, 0.06, 0.07, -0.01, 0.08], 0.0) SELECT s;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f > 0.0, "Sortino ratio should be positive here, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_sortino_ratio_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE s = SORTINO_RATIO([0.05, 0.06, 0.07, -0.01, 0.08], 0.0) SELECT s;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("s") {
        Some(Value::Float(f)) => assert!(*f > 0.0, "SORTINO_RATIO alias should give positive result, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_drawdown_monotonically_increasing_is_zero() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE d = DRAWDOWN([100.0, 110.0, 120.0, 130.0, 140.0]) SELECT d;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("d") {
        Some(Value::Float(f)) => assert!(*f == 0.0, "monotonically increasing prices should have 0 drawdown, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_max_drawdown_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE d = MAX_DRAWDOWN([100.0, 110.0, 120.0, 130.0, 140.0]) SELECT d;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("d") {
        Some(Value::Float(f)) => assert!(*f == 0.0, "MAX_DRAWDOWN alias: monotonically increasing → 0, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_drawdown_known_drop() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("d") {
        Some(Value::Float(f)) => assert!((*f - 0.5).abs() < 1e-9, "expected 0.5 drawdown, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_beta_coeff_identical_series_is_one() {
    ...
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => assert!((*f - 1.0).abs() < 1e-9, "identical series should give beta=1, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_portfolio_beta_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => assert!((*f - 1.0).abs() < 1e-9, "PORTFOLIO_BETA alias: identical series → 1, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_information_ratio_same_returns_is_zero() {
    ...
    match r.rows[0].data.get("ir") {
        ...
        Some(Value::Float(f)) => assert!(f.abs() < 1e-9, "IR with identical series should be ~0 or Null, got {}", f),
...
fn test_ir_ratio_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("ir") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "IR_RATIO should return a finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_value_at_risk_returns_positive_float() {
    ...
    match r.rows[0].data.get("v") {
        Some(Value::Float(f)) => assert!(*f > 0.0 && f.is_finite(), "VaR should be a positive finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_var_p_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("v") {
        Some(Value::Float(f)) => assert!(*f > 0.0 && f.is_finite(), "VAR_P alias should return positive finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_expected_shortfall_gte_var() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let es = match r.rows[0].data.get("es") { Some(Value::Float(f)) => *f, other => panic!("expected Float for ES, got {:?}", other) };
    let v = match r.rows[0].data.get("v") { Some(Value::Float(f)) => *f, other => panic!("expected Float for VaR, got {:?}", other) };
    assert!(es >= v - 1e-9, "Expected Shortfall ({}) should be >= VaR ({}) at same confidence", es, v);
}
...
fn test_cvar_alias() {
    ...
    match r.rows[0].data.get("es") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "CVaR alias should return finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_hpr_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("h") {
        Some(Value::Float(f)) => assert!((*f - 0.1).abs() < 1e-9, "HPR alias: expected 0.1, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_hpr_with_dividend() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("h") {
        Some(Value::Float(f)) => assert!((*f - 0.15).abs() < 1e-9, "expected HPR with dividend=0.15, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_annualized_return_365_days() {
    ...
    match r.rows[0].data.get("a") {
        Some(Value::Float(f)) => assert!((*f - 0.1).abs() < 1e-9, "ANNUALIZED_RETURN(0.1, 365) should be 0.1, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_ann_ret_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("a") {
        Some(Value::Float(f)) => assert!((*f - 0.1).abs() < 1e-9, "ANN_RET alias: expected 0.1, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_annualized_return_half_year() {
    ...
    match r.rows[0].data.get("a") {
        Some(Value::Float(f)) => {
            ...
            assert!((*f - expected).abs() < 0.001, "expected ~{}, got {}", expected, f);
        }
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_calmar_ratio_basic() {
    ...
    let mut p = Parser::new("QUERY t COMPUTE c = CALMAR([100.0, 150.0, 120.0, 180.0], 3.0) SELECT c;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("c") {
        Some(Value::Float(f)) => assert!(*f > 0.0 && f.is_finite(), "CALMAR ratio should be positive finite, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_calmar_ratio_alias() {
    let (_dir, db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE c = CALMAR_RATIO([100.0, 150.0, 120.0, 180.0], 3.0) SELECT c;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("c") {
        Some(Value::Float(f)) => assert!(*f > 0.0 && f.is_finite(), "CALMAR_RATIO alias should be positive finite, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_treynor_ratio_basic() {
    ...
    match r.rows[0].data.get("t") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "TREYNOR_RATIO should return finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
...
fn test_treynor_alias() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("t") {
        Some(Value::Float(f)) => assert!(f.is_finite(), "TREYNOR alias should return finite float, got {}", f),
        other => panic!("expected Float, got {:?}", other),
    }
}
