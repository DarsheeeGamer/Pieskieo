...
fn test_date_trunc() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"ts": 1710510645_i64})).unwrap();
    ...
    match r.rows[0].data.get("d") {
        Some(Value::Integer(v)) => {
            ...
            assert_eq!(v % 86400, 0, "DATE_TRUNC('day') result should be divisible by 86400");
        }
...
fn test_date_part() {
    ...
    let mut p = Parser::new("QUERY t COMPUTE h = DATE_PART(\"hour\", ts) COMPUTE q = QUARTER(ts) SELECT h, q;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_day_of_week_and_weekend() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"ts": 0_i64})).unwrap();

    let mut p = Parser::new("QUERY t COMPUTE dow = DAY_OF_WEEK(ts) COMPUTE is_w = IS_WEEKDAY(ts) SELECT dow, is_w;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    // Thursday = 4 (0=Sunday)
...
fn test_week_of_year() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"ts": 1704585600_i64})).unwrap(); // approx 2024-01-07
