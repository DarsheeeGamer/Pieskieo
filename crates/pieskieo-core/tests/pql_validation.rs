...
fn test_is_valid_date_leap_year_valid() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d1": "2024-02-29", "d2": "2023-02-29", "d3": "invalid"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE v1 = IS_VALID_DATE(d1) COMPUTE v2 = IS_VALID_DATE(d2) COMPUTE v3 = IS_VALID_DATE(d3) SELECT v1, v2, v3;"#);
...
fn test_is_valid_date_month_bounds() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d1": "2024-04-30", "d2": "2024-04-31"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_VALID_DATE(d1) COMPUTE bad = IS_VALID_DATE(d2) SELECT ok, bad;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_validate_date_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2025-12-31"})).unwrap();
...
fn test_is_valid_date_invalid_month() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2024-13-01"})).unwrap();
...
fn test_is_valid_datetime_full() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dt": "2024-06-15 23:59:59"})).unwrap();
...
fn test_is_valid_datetime_date_only() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dt": "2024-01-01"})).unwrap();
...
fn test_is_valid_datetime_bad_time() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dt": "2024-01-01 25:00:00"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_VALID_DATETIME(dt) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_is_date_before() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"a": "2022-01-01", "b": "2023-06-15"})).unwrap();
...
fn test_date_lt_alias_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"a": "2025-12-31", "b": "2023-01-01"})).unwrap();
...
fn test_is_date_after() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"a": "2025-06-01", "b": "2020-01-01"})).unwrap();
...
fn test_is_date_between_inclusive() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2024-06-15"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_DATE_BETWEEN(d, "2024-01-01", "2024-12-31") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(true)));
}
...
fn test_date_in_range_out() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2025-01-01"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = DATE_IN_RANGE(d, "2024-01-01", "2024-12-31") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_is_future_date() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2030-01-01"})).unwrap();
...
fn test_is_past_date() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2020-06-01"})).unwrap();
...
fn test_is_in_future_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"d": "2019-01-01"})).unwrap();
...
fn test_is_in_range_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 50})).unwrap();
...
fn test_is_in_range_boundary() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 100})).unwrap();
...
fn test_is_in_range_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 101})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_IN_RANGE(n, 1, 100) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_is_whole_integer() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 5})).unwrap();
...
fn test_is_whole_float_fractional() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 5.5})).unwrap();
...
fn test_multiple_of_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 9})).unwrap();
...
fn test_is_prime_number_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 17})).unwrap();
...
fn test_is_prime_number_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 18})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_PRIME_NUMBER(n) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_matches_pattern_star() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "hello world"})).unwrap();
...
fn test_matches_pattern_question_mark() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "cat"})).unwrap();
...
fn test_matches_pattern_no_match() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "dog"})).unwrap();
...
fn test_is_alpha_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "HelloWorld"})).unwrap();
...
fn test_is_alphabetic_false_with_digit() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "Hello1"})).unwrap();
...
fn test_is_digits_only_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "123456"})).unwrap();
...
fn test_is_all_digits_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "123abc"})).unwrap();
...
fn test_has_lowercase_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "ALLCAPS"})).unwrap();
...
fn test_has_digit_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "pass1word"})).unwrap();
...
fn test_has_special_char_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "p@ssword"})).unwrap();
...
fn test_is_strong_password_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"pw": "Abc123!@#"})).unwrap();
...
fn test_is_strong_pass_too_short() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"pw": "Ab1!"})).unwrap();
...
fn test_is_strong_password_no_special() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"pw": "Abcdef12"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_STRONG_PASSWORD(pw) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_is_empty_or_null_empty_string() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "   "})).unwrap();
...
fn test_is_blank_val_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "hello"})).unwrap();
...
fn test_is_not_empty_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "data"})).unwrap();
...
fn test_string_matches_any_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "foobar", "patterns": ["foo*", "baz*"]})).unwrap();
...
fn test_matches_any_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "qux", "patterns": ["foo*", "bar*"]})).unwrap();
...
fn test_assert_not_null_passes_value() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"num": 42})).unwrap();
...
fn test_require_not_null_error_string() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"missing": null})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = REQUIRE_NOT_NULL(missing, "missing") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_assert_in_range_passes() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"score": 85})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = ASSERT_IN_RANGE(score, 0, 100, "score") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_require_in_range_error() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"score": 150})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = REQUIRE_IN_RANGE(score, 0, 100, "score") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("ok") {
        Some(Value::String(s)) => assert!(s.contains("VALIDATION_ERROR")),
        other => panic!("expected validation error string, got {:?}", other),
...
fn test_coalesce_default_uses_default() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"x": null})).unwrap();
...
fn test_coalesce_default_returns_value() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"x": 42})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = COALESCE_DEFAULT(x, 99) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Integer(42)));
}
...
fn test_nullif_empty_replaces_empty_string() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("default".to_string())));
}
...
fn test_nullif_empty_keeps_non_empty() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = NULLIF_EMPTY(s, "default") SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("hello".to_string())));
}
...
fn test_nullif_equal_returns_null() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"n": 0})).unwrap();
...
fn test_get_type_integer() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("integer".to_string())));
}
...
fn test_get_type_string() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("string".to_string())));
}

#[test]
fn test_get_type_bool() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"b": true})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = GET_TYPE(b) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("bool".to_string())));
}
...
fn test_cast_safe_string_to_integer() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "42"})).unwrap();
...
fn test_try_cast_invalid_returns_null() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "not_a_number"})).unwrap();
...
fn test_cast_safe_integer_to_string() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::String("100".to_string())));
}

#[test]
fn test_cast_safe_string_to_bool() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "true"})).unwrap();
...
fn test_validate_schema_pass() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rec": {"age": 25, "name": "Alice"}})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = VALIDATE_SCHEMA(rec, {"age": "integer", "name": "string"}) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(true)));
}
...
fn test_matches_schema_fail_wrong_type() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rec": {"age": "twenty-five", "name": "Alice"}})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = MATCHES_SCHEMA(rec, {"age": "integer", "name": "string"}) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_array_is_sorted_asc() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1, 2, 3, 4, 5]})).unwrap();
...
fn test_is_sorted_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1, 3, 2, 4]})).unwrap();
...
fn test_array_is_sorted_desc() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [5, 4, 3, 2, 1]})).unwrap();
...
fn test_array_is_unique_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1, 2, 3, 4]})).unwrap();
...
fn test_all_satisfy_even_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [2, 4, 5]})).unwrap();
...
fn test_any_satisfy_odd_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [2, 4, 6]})).unwrap();
...
fn test_array_count_if_even() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1, 2, 3, 4, 5, 6]})).unwrap();
...
fn test_count_where_positive() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [-1, 2, -3, 4, 0]})).unwrap();
...
fn test_array_has_nulls_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1, null, 3]})).unwrap();
...
fn test_array_null_count() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [null, 1, null, 2, null]})).unwrap();
...
fn test_array_non_null_count() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [null, 1, null, 2]})).unwrap();
...
fn test_count_non_nulls_full_array() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [10, 20, 30]})).unwrap();
...
fn test_is_alnum_true() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "abc123"})).unwrap();
...
fn test_is_alnum_false_with_special() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "abc!"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE ok = IS_ALNUM(s) SELECT ok;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ok"), Some(&Value::Bool(false)));
}
...
fn test_contains_special_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "no_special"})).unwrap();
