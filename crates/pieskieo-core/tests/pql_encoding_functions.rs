...
fn test_base64_encode_world() {
    // "world" -> "d29ybGQ="
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "world"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = BASE64_ENCODE(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r"), Some(&Value::String("d29ybGQ=".to_string())));
}
...
fn test_base64_decode_hello() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "aGVsbG8="})).unwrap();
...
fn test_base64_roundtrip() {
    ...
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "hello"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = BASE64_ENCODE(s) COMPUTE dec = BASE64_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_url_encode_special_chars() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "a=1&b=2"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = URL_ENCODE(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_url_encode_safe_chars() {
    ...
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "abc-123_test.ok~"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = URL_ENCODE(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r"), Some(&Value::String("abc-123_test.ok~".to_string())));
}
...
fn test_url_decode_space() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "hello%20world"})).unwrap();
...
fn test_url_encode_decode_roundtrip() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "foo bar+baz"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = URL_ENCODE(s) COMPUTE dec = URL_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("dec"), Some(&Value::String("foo bar+baz".to_string())));
}
...
fn test_html_encode_angle_brackets() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "<b>"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = HTML_ENCODE(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r"), Some(&Value::String("&lt;b&gt;".to_string())));
}
...
fn test_html_encode_ampersand_and_quote() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "a & \"b\""})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = HTML_ENCODE(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_html_encode_decode_roundtrip() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "<div class=\"x\">a & b</div>"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = HTML_ENCODE(s) COMPUTE dec = HTML_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("dec"), Some(&Value::String("<div class=\"x\">a & b</div>".to_string())));
}
...
fn test_rle_encode_integers() {
    // [1,1,1,2,2] -> [[1,3],[2,2]]
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1,1,1,2,2]})).unwrap();
...
fn test_run_length_encode_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [5,5,5]})).unwrap();
...
fn test_rle_encode_empty() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": []})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = RLE_ENCODE(arr) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_rle_decode_integers() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"arr": [[1,3],[2,2]]})).unwrap();
...
fn test_run_length_decode_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"arr": [[7,2]]})).unwrap();
...
fn test_rle_encode_decode_roundtrip() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"arr": [3,3,1,1,1,2]})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = RLE_ENCODE(arr) COMPUTE dec = RLE_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
...
fn test_delta_encode_basic() {
    // [1,3,6,10] -> [1,2,3,4]
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [1,3,6,10]})).unwrap();
...
fn test_delta_compress_alias() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::Array(vec![
            Value::Integer(10), Value::Integer(10), Value::Integer(10),
        ]))
    );
}
...
fn test_delta_decode_basic() {
    ...
    assert_eq!(
        result.rows[0].data.get("r"),
        Some(&Value::Array(vec![
            Value::Integer(1), Value::Integer(3), Value::Integer(6), Value::Integer(10),
        ]))
    );
}
...
fn test_delta_decompress_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [10,10,10]})).unwrap();
...
fn test_delta_encode_decode_roundtrip() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [5,8,12,15,20]})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = DELTA_ENCODE(arr) COMPUTE dec = DELTA_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("dec"),
        Some(&Value::Array(vec![
            Value::Integer(5), Value::Integer(8), Value::Integer(12),
...
fn test_zigzag_encode_decode_roundtrip() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE enc = ZIGZAG_ENCODE(n) COMPUTE dec = ZIGZAG_DECODE(enc) SELECT dec;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_bit_pack_returns_string() {
    ...
    match result.rows[0].data.get("r") {
        Some(Value::String(s)) => {
            ...
            assert!(s.chars().all(|c| c.is_ascii_hexdigit()), "BIT_PACK output must be hex");
        }
...
fn test_pack_bits_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [0,1]})).unwrap();
...
fn test_bit_pack_unpack_roundtrip() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [5,10,15]})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE packed = BIT_PACK(arr, 8) COMPUTE unpacked = BIT_UNPACK(packed, 8, 3) SELECT unpacked;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("unpacked"),
        Some(&Value::Array(vec![
            Value::Integer(5), Value::Integer(10), Value::Integer(15),
        ]))
    );
}
...
fn test_unpack_bits_alias() {
    ...
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"arr": [7,8]})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE packed = PACK_BITS(arr, 8) COMPUTE unpacked = UNPACK_BITS(packed, 8, 2) SELECT unpacked;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        result.rows[0].data.get("unpacked"),
...
fn test_rot13_hello() {
    ...
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r"), Some(&Value::String("uryyb".to_string())));
}
...
fn test_rot13_double_application() {
    ...
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r2"), Some(&Value::String("hello".to_string())));
}
...
fn test_rot13_preserves_non_alpha() {
    ...
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"s": "hello123!"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE r = ROT13(s) SELECT r;"#);
    let result = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(result.rows[0].data.get("r"), Some(&Value::String("uryyb123!".to_string())));
}
