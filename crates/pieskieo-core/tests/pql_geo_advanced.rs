...
fn test_geo_bearing() {
    ...
    match r.rows[0].data.get("b") {
        Some(Value::Float(f)) => assert!(*f >= 0.0 && *f < 360.0, "bearing should be [0,360), got {}", f),
...
fn test_geo_midpoint() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"lat1": 0.0, "lon1": 0.0, "lat2": 0.0, "lon2": 90.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE m = GEO_MIDPOINT(lat1, lon1, lat2, lon2) SELECT m;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_geo_bbox() {
    ...
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"lat1": 10.0, "lon1": 20.0, "lat2": 30.0, "lon2": 40.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE bbox = GEO_BBOX(lat1, lon1, lat2, lon2) SELECT bbox;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_geo_within_radius() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"lat": 40.7128, "lon": -74.0060})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE near = GEO_WITHIN_RADIUS(lat, lon, 40.72, -74.01, 10) COMPUTE far = GEO_WITHIN_RADIUS(lat, lon, 51.5074, -0.1278, 100) SELECT near, far;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("near"), Some(&Value::Bool(true)), "should be within 10km");
    assert_eq!(r.rows[0].data.get("far"), Some(&Value::Bool(false)), "should not be within 100km of London");
}
...
fn test_geohash_encode() {
    ...
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"lat": 51.5074, "lon": -0.1278})).unwrap();
    ...
    match r.rows[0].data.get("h") {
        Some(Value::String(s)) => {
            ...
            assert!(s.starts_with('g') || s.starts_with('e') || s.len() == 5,
                "unexpected geohash: {}", s);
        }
...
fn test_geo_distance_km() {
    ...
    let mut p = Parser::new(r#"QUERY t COMPUTE d = GEO_DISTANCE_KM(lat1, lon1, lat2, lon2) SELECT d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("d") {
        Some(Value::Float(f)) => assert!(*f > 5000.0 && *f < 6000.0, "NYC-London ~5570 km, got {}", f),
        other => panic!("expected float, got {:?}", other),
    }
}
