...
fn test_poly_eval() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"coeffs": [1,2,3], "xval": 2})).unwrap();
...
fn test_polynomial_eval_alias() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"coeffs": [3,0,1], "xval": 4})).unwrap();
...
fn test_poly_derive() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"coeffs": [1,2,3]})).unwrap();
...
fn test_poly_integrate() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"coeffs": [2,6]})).unwrap();
...
fn test_catalan_number() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 4})).unwrap();
...
fn test_catalan_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 0})).unwrap();
...
fn test_digital_root() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 493})).unwrap();
...
fn test_aliquot_sum() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 6})).unwrap();
...
fn test_euler_totient() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 9})).unwrap();
...
fn test_totient_alias() {
    let (db, ex) = setup();
    // phi(12) = 4
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 12})).unwrap();
...
fn test_is_perfect() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"p": 28, "np": 10})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE a = IS_PERFECT(p) COMPUTE b = IS_PERFECT(np) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_abundant() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"abn": 12, "not_abn": 5})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE a = IS_ABUNDANT(abn) COMPUTE b = IS_ABUNDANT(not_abn) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Bool(true)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Bool(false)));
}
...
fn test_is_deficient() {
    let (db, ex) = setup();
    // 5 is deficient (1<5)
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"dfn": 5, "not_dfn": 12})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE a = IS_DEFICIENT(dfn) COMPUTE b = IS_DEFICIENT(not_dfn) SELECT a, b;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("a"), Some(&Value::Bool(true)));
    assert_eq!(r.rows[0].data.get("b"), Some(&Value::Bool(false)));
}
...
fn test_collatz_length() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 1})).unwrap();
...
fn test_collatz_length_27() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 27})).unwrap();
...
fn test_tribonacci() {
    let (db, ex) = setup();
    // T(7) = 24
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 7})).unwrap();
...
fn test_pell_number() {
    let (db, ex) = setup();
    // P(5) = 29
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 5})).unwrap();
...
fn test_sylvester_seq() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 3})).unwrap();
...
fn test_continued_fraction() {
    let (db, ex) = setup();
    // 7/3 = [2; 3] -> [2, 3]
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"pv": 7, "qv": 3})).unwrap();
...
fn test_factorize_alias() {
    let (db, ex) = setup();
    // 13 is prime -> [13]
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nval": 13})).unwrap();
