...
fn test_npv() {
    ...
    match r.rows[0].data.get("n") {
        Some(Value::Float(f)) => assert!(*f > 800.0 && *f < 1200.0, "NPV of [400,400,400] at 10% should be ~994, got {}", f),
...
fn test_cagr() {
    ...
    let ex = Executor::new(db.clone());
    // CAGR(1000, 1610, 5) ≈ 10%
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"start_val": 1000.0, "end_val": 1610.51, "years": 5.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE c = CAGR(start_val, end_val, years) SELECT c;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("c") {
        Some(Value::Float(f)) => assert!((*f - 0.10).abs() < 0.01, "CAGR should be ~0.10, got {}", f),
        other => panic!("expected float, got {:?}", other),
    }
}
...
fn test_roi() {
    ...
    let ex = Executor::new(db.clone());
    // ROI(150, 100) = 0.5 (50%)
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"gain": 150.0, "cost": 100.0})).unwrap();
...
fn test_compound_interest() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"principal": 1000.0, "rate": 0.10, "t": 1.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE a = COMPOUND_INTEREST(principal, rate, t) SELECT a;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_profit_margin() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"revenue": 100.0, "cost": 70.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE m = PROFIT_MARGIN(revenue, cost) SELECT m;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("m") {
        Some(Value::Float(f)) => assert!((*f - 0.3).abs() < 0.001, "margin should be 0.3, got {}", f),
        other => panic!("expected float, got {:?}", other),
    }
}
...
fn test_pmt_pv() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"rate": 0.05, "n": 12, "pv": 10000.0})).unwrap();
...
fn test_depreciation_sl() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"cost": 10000.0, "salvage": 1000.0, "life": 9.0})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE d = DEPRECIATION_SL(cost, salvage, life) SELECT d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("d") {
        Some(Value::Float(f)) => assert!((*f - 1000.0).abs() < 0.01, "depreciation should be 1000, got {}", f),
        other => panic!("expected float, got {:?}", other),
    }
}
