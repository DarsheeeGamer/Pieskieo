...
fn test_is_leap_year_2000() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 2000})).unwrap();
...
fn test_is_leap_year_1900_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 1900})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_LEAP_YEAR(yr) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_is_leap_year_2024() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 2024})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_LEAP_YEAR(yr) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Bool(true)));
}
...
fn test_is_leap_year_2023_false() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 2023})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = IS_LEAP_YEAR(yr) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::Bool(false)));
}
...
fn test_days_in_month_feb_leap() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 2000, "mo": 2})).unwrap();
...
fn test_days_in_month_feb_non_leap() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 1900, "mo": 2})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = DAYS_IN_MONTH(yr, mo) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_days_in_month_january() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"yr": 2024, "mo": 1})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = DAYS_IN_MONTH(yr, mo) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_last_day_of_month_feb_leap() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-02-15"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = LAST_DAY_OF_MONTH(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-02-29".to_string())));
}
...
fn test_last_day_of_month_jan() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2023-01-01"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = LAST_DAY_OF_MONTH(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2023-01-31".to_string())));
}
...
fn test_first_day_of_month() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-02-01".to_string())));
}
...
fn test_quarter_number_q3() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-07-10"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = QUARTER_NUMBER(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_quarter_number_q4() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-10-01"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = QUARTER_NUMBER(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_quarter_start() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-05-15"})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = QUARTER_START(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-04-01".to_string())));
}
...
fn test_quarter_end() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-06-30".to_string())));
}
...
fn test_date_add_months_clamp() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2024-01-31", "mo": 1})).unwrap();
...
fn test_date_add_years_clamp() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"dt": "2020-02-29", "yr": 1})).unwrap();
    ...
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2021-02-28".to_string())));
}
...
fn test_day_of_week_name_monday() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("Monday".to_string())));
}
...
fn test_day_of_week_name_saturday() {
    ...
    let mut p = Parser::new("QUERY t COMPUTE nv = DAY_OF_WEEK_NAME(dt) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("Saturday".to_string())));
}
...
fn test_month_name_january() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"mo": 1})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = MONTH_NAME(mo) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("January".to_string())));
}
...
fn test_month_name_december() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"mo": 12})).unwrap();
    let mut p = Parser::new("QUERY t COMPUTE nv = MONTH_NAME(mo) SELECT nv;");
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("December".to_string())));
}
...
fn test_date_diff_months() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"d1": "2024-01-01", "d2": "2024-06-01"})).unwrap();
...
fn test_date_diff_years() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"d1": "2020-01-01", "d2": "2024-01-01"})).unwrap();
...
fn test_next_weekday_from_friday() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-01-08".to_string())));
}
...
fn test_prev_weekday_from_monday() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("2024-01-05".to_string())));
}
...
fn test_biz_days_between_mon_to_fri() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"d1": "2024-01-08", "d2": "2024-01-12"})).unwrap();
