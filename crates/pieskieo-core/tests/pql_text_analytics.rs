...
fn test_sentence_count_three() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Hello. World? Foo!"})).unwrap();
...
fn test_count_sentences_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "First sentence. Second sentence!"})).unwrap();
...
fn test_avg_word_length() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "ab cde"})).unwrap();
...
fn test_mean_word_len_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "hi there"})).unwrap();
...
fn test_longest_word() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "hello world programming"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE lw = LONGEST_WORD(txt) SELECT lw;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("lw"), Some(&Value::String("programming".to_string())));
}
...
fn test_max_word_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "cat elephant dog"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE lw = MAX_WORD(txt) SELECT lw;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("lw"), Some(&Value::String("elephant".to_string())));
}
...
fn test_most_common_word() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "the cat sat on the mat the"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE mw = MOST_COMMON_WORD(txt) SELECT mw;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("mw"), Some(&Value::String("the".to_string())));
}
...
fn test_top_word_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "foo bar foo baz foo"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE mw = TOP_WORD(txt) SELECT mw;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("mw"), Some(&Value::String("foo".to_string())));
}
...
fn test_word_frequency() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "a b a"})).unwrap();
...
fn test_word_freq_map_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "x y x x"})).unwrap();
...
fn test_reading_time_sec_200_words() {
    ...
    let words: String = std::iter::repeat("word").take(200).collect::<Vec<_>>().join(" ");
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": words})).unwrap();
...
fn test_read_time_secs_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "one two three four five"})).unwrap();
...
fn test_flesch_reading_ease_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "This is a simple text. It is easy to read!"})).unwrap();
...
fn test_fog_index_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Simple text here. Easy words only!"})).unwrap();
...
fn test_clean_text_removes_punctuation() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("ct"), Some(&Value::String("hello, world!".to_string())));
}
...
fn test_extract_emails() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Contact user@example.com for info"})).unwrap();
...
fn test_find_emails_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Send to admin@test.org and info@corp.com"})).unwrap();
...
fn test_extract_hashtags() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "#rust is cool #programming"})).unwrap();
...
fn test_find_hashtags_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Love #cats and #dogs today"})).unwrap();
...
fn test_extract_mentions() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "@alice and @bob are here"})).unwrap();
...
fn test_find_mentions_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Hello @charlie how are you"})).unwrap();
...
fn test_tf_idf_score_positive() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tc": 3, "nv": 100, "nd": 1000, "df": 50})).unwrap();
...
fn test_tf_idf_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"tc": 5, "nv": 50, "nd": 100, "df": 10})).unwrap();
...
fn test_text_similarity_identical() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "cat dog", "t2": "cat dog"})).unwrap();
...
fn test_cosine_sim_text_disjoint() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"t1": "apple orange banana", "t2": "car bus train"})).unwrap();
...
fn test_sentiment_simple_positive() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "I love this great product"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE sn = SENTIMENT_SIMPLE(txt) SELECT sn;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sn"), Some(&Value::String("positive".to_string())));
}
...
fn test_simple_sentiment_alias_negative() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "This is terrible and bad"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE sn = SIMPLE_SENTIMENT(txt) SELECT sn;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sn"), Some(&Value::String("negative".to_string())));
}
...
fn test_sentiment_simple_neutral() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "The box contains items"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE sn = SENTIMENT_SIMPLE(txt) SELECT sn;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("sn"), Some(&Value::String("neutral".to_string())));
}
...
fn test_remove_stopwords() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "the cat sat on the mat"})).unwrap();
...
fn test_strip_stopwords_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "a quick brown fox"})).unwrap();
...
fn test_normalize_text() {
    ...
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("hello world".to_string())));
}
...
fn test_text_normalize_alias() {
    let (db, ex) = setup();
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(),
        serde_json::json!({"txt": "Rust   Is   AWESOME"})).unwrap();
    let mut p = Parser::new(r#"QUERY t COMPUTE nv = TEXT_NORMALIZE(txt) SELECT nv;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("nv"), Some(&Value::String("rust is awesome".to_string())));
}
