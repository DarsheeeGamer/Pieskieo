...
fn test_object_entries() {
    ...
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"data": {"x": 1, "y": 2}})).unwrap();
...
fn test_pick() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"profile": {"name": "Alice", "age": 30, "secret": "hidden"}})).unwrap();
...
fn test_omit() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"data": {"a": 1, "b": 2, "c": 3}})).unwrap();
...
fn test_object_size() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"obj": {"a": 1, "b": 2, "c": 3}})).unwrap();
...
fn test_object_has() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"obj": {"x": 42}})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE yes = OBJECT_HAS(obj, "x") COMPUTE no = OBJECT_HAS(obj, "z") SELECT yes, no;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_object_get_with_default() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"cfg": {"color": "blue"}})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE c = OBJECT_GET(cfg, "color", "red") COMPUTE d = OBJECT_GET(cfg, "size", "medium") SELECT c, d;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(r.rows[0].data.get("c"), Some(&Value::String("blue".to_string())));
    assert_eq!(r.rows[0].data.get("d"), Some(&Value::String("medium".to_string())));
}
...
fn test_invert_object() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"mapping": {"en": "hello", "es": "hola"}})).unwrap();
...
fn test_flatten_object() {
    ...
    let db = Arc::new(PieskieoDb::open(dir.path()).unwrap());
    let ex = Executor::new(db.clone());
    db.put_doc_ns(None, Some("t"), Uuid::new_v4(), serde_json::json!({"nested": {"a": {"b": 42}}})).unwrap();

    let mut p = Parser::new(r#"QUERY t COMPUTE f = FLATTEN_OBJECT(nested, ".") SELECT f;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("f") {
        Some(Value::Object(obj)) => {
            assert!(obj.contains_key("a.b"), "should have flattened key 'a.b', got: {:?}", obj.keys().collect::<Vec<_>>());
