...
fn test_rolling_sum() {
    ...
    let mut p = Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE rs = ROLLING_SUM(val, 3) SELECT rs;"#);
...
fn test_cummax_cummin() {
    ...
    let mut p = Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE cmax = CUMMAX(val) COMPUTE cmin = CUMMIN(val) SELECT cmax, cmin;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_expanding_mean() {
    ...
    let mut p = Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE em = EXPANDING_MEAN(val) SELECT em;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("em") {
        Some(Value::Array(arr)) => {
            assert_eq!(arr.len(), 3);
            // Verify the means are all floats and the final cumulative mean = (10+20+30)/3 = 20.0
            let floats: Vec<f64> = arr.iter().filter_map(|v| if let Value::Float(f) = v { Some(*f) } else { None }).collect();
            ...
            assert!((floats.last().unwrap() - 20.0).abs() < 0.001, "final cumulative mean should be 20.0, got {}", floats.last().unwrap());
            // All values should be between the min (10) and max (30) of the series
            assert!(floats.iter().all(|&f| f >= 10.0 && f <= 30.0), "all cumulative means should be within [10, 30]");
        }
        other => panic!("expected array, got {:?}", other),
    }
}
...
fn test_zscore_series() {
    ...
    for _ in 0..4 {
        db.put_doc_ns(None, Some("ts"), Uuid::new_v4(), serde_json::json!({"val": 5})).unwrap();
    }

    let mut p = Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE z = ZSCORE_SERIES(val) SELECT z;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("z") {
        Some(Value::Array(arr)) => {
            assert_eq!(arr.len(), 4);
            assert!(arr.iter().all(|v| v == &Value::Float(0.0)), "all z-scores should be 0 for constant series");
        }
        other => panic!("expected array, got {:?}", other),
    }
}
...
fn test_expanding_sum() {
    ...
    let mut p = Parser::new(r#"QUERY ts COMPUTE g = 1 GROUP BY g COMPUTE es = EXPANDING_SUM(val) SELECT es;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
