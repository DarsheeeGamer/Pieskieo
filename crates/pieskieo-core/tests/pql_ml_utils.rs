...
fn test_one_hot_encode_basic() {
    ...
    let arr = get_array(
        &ex,
        "t_ohe1",
        "QUERY t_ohe1 COMPUTE res = ONE_HOT_ENCODE(idx, nc) SELECT res;",
        "res",
    );
    assert_eq!(
        arr.len(),
        4,
        "ONE_HOT_ENCODE(1,4) should produce 4 elements"
    );
    ...
    assert_eq!(
        vals,
        vec![0, 1, 0, 0],
        "ONE_HOT_ENCODE(1,4) should be [0,1,0,0]"
    );
}
...
fn test_one_hot_encode_first_class() {
    ...
    let arr = get_array(
        &ex,
        "t_ohe2",
        "QUERY t_ohe2 COMPUTE res = ONE_HOT_ENCODE(idx, nc) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 3);
...
fn test_one_hot_alias() {
    ...
    let arr = get_array(
        &ex,
        "t_ohe3",
        "QUERY t_ohe3 COMPUTE res = ONE_HOT(idx, nc) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 4);
...
fn test_softmax_sums_to_one() {
    ...
    let arr = get_array(
        &ex,
        "t_sm1",
        "QUERY t_sm1 COMPUTE res = SOFTMAX(arr) SELECT res;",
        "res",
    );
...
fn test_softmax_array_alias() {
    ...
    let arr1 = get_array(
        &ex,
        "t_sm2",
        "QUERY t_sm2 COMPUTE res = SOFTMAX(arr) SELECT res;",
        "res",
    );
    let arr2 = get_array(
        &ex,
        "t_sm2",
        "QUERY t_sm2 COMPUTE res = SOFTMAX_ARRAY(arr) SELECT res;",
        "res",
    );
    assert_eq!(arr1.len(), arr2.len());
    for (a, b) in arr1.iter().zip(arr2.iter()) {
        assert!(
            (val_to_f64(a) - val_to_f64(b)).abs() < 1e-10,
            "SOFTMAX and SOFTMAX_ARRAY should agree"
        );
    }
}
...
fn test_softmax_all_elements_positive() {
    ...
    let arr = get_array(
        &ex,
        "t_sm3",
        "QUERY t_sm3 COMPUTE res = SOFTMAX(arr) SELECT res;",
        "res",
    );
    for v in &arr {
        assert!(
...
fn test_sigmoid_at_zero() {
    ...
    let f = get_float(
        &ex,
        "t_sig1",
        "QUERY t_sig1 COMPUTE res = SIGMOID(xval) SELECT res;",
        "res",
    );
    assert!(
        (f - 0.5).abs() < 1e-9,
...
fn test_logistic_alias() {
    ...
    let a = get_float(
        &ex,
        "t_sig2",
        "QUERY t_sig2 COMPUTE res = SIGMOID(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_sig2",
        "QUERY t_sig2 COMPUTE res = LOGISTIC(xval) SELECT res;",
        "res",
    );
...
fn test_sigmoid_large_positive() {
    ...
    let f = get_float(
        &ex,
        "t_sig3",
        "QUERY t_sig3 COMPUTE res = SIGMOID(xval) SELECT res;",
        "res",
    );
    assert!(
        (f - 1.0).abs() < 1e-9,
...
fn test_relu_negative_input() {
    ...
    let f = get_float(
        &ex,
        "t_relu1",
        "QUERY t_relu1 COMPUTE res = RELU(xval) SELECT res;",
        "res",
    );
...
fn test_relu_positive_input() {
    ...
    let f = get_float(
        &ex,
        "t_relu2",
        "QUERY t_relu2 COMPUTE res = RELU(xval) SELECT res;",
        "res",
    );
...
fn test_relu_activation_alias() {
    ...
    let a = get_float(
        &ex,
        "t_relu3",
        "QUERY t_relu3 COMPUTE res = RELU(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_relu3",
        "QUERY t_relu3 COMPUTE res = RELU_ACTIVATION(xval) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "RELU and RELU_ACTIVATION should agree"
    );
}
...
fn test_leaky_relu_negative_default_alpha() {
    ...
    let f = get_float(
        &ex,
        "t_lrelu1",
        "QUERY t_lrelu1 COMPUTE res = LEAKY_RELU(xval) SELECT res;",
        "res",
    );
    assert!(
        (f - (-0.02)).abs() < 1e-9,
        "LEAKY_RELU(-2.0) should be -0.02, got {}",
        f
    );
}
...
fn test_lrelu_alias() {
    ...
    let a = get_float(
        &ex,
        "t_lrelu2",
        "QUERY t_lrelu2 COMPUTE res = LEAKY_RELU(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_lrelu2",
        "QUERY t_lrelu2 COMPUTE res = LRELU(xval) SELECT res;",
        "res",
    );
...
fn test_elu_negative_input() {
    ...
    let f = get_float(
        &ex,
        "t_elu1",
        "QUERY t_elu1 COMPUTE res = ELU(xval) SELECT res;",
        "res",
    );
    let expected = (-1.0_f64).exp() - 1.0;
    assert!(
        (f - expected).abs() < 1e-9,
        "ELU(-1.0) should be {}, got {}",
        expected,
        f
    );
}
...
fn test_elu_activation_alias() {
    ...
    let a = get_float(
        &ex,
        "t_elu2",
        "QUERY t_elu2 COMPUTE res = ELU(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_elu2",
        "QUERY t_elu2 COMPUTE res = ELU_ACTIVATION(xval) SELECT res;",
        "res",
    );
...
fn test_gelu_at_zero() {
    ...
    let f = get_float(
        &ex,
        "t_gelu1",
        "QUERY t_gelu1 COMPUTE res = GELU(xval) SELECT res;",
        "res",
    );
...
fn test_gelu_activation_alias() {
    ...
    let a = get_float(
        &ex,
        "t_gelu2",
        "QUERY t_gelu2 COMPUTE res = GELU(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_gelu2",
        "QUERY t_gelu2 COMPUTE res = GELU_ACTIVATION(xval) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "GELU and GELU_ACTIVATION should agree"
    );
}
...
fn test_swish_at_zero() {
    ...
    let f = get_float(
        &ex,
        "t_swish1",
        "QUERY t_swish1 COMPUTE res = SWISH(xval) SELECT res;",
        "res",
    );
...
fn test_swish_activation_alias() {
    ...
    let a = get_float(
        &ex,
        "t_swish2",
        "QUERY t_swish2 COMPUTE res = SWISH(xval) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_swish2",
        "QUERY t_swish2 COMPUTE res = SWISH_ACTIVATION(xval) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "SWISH and SWISH_ACTIVATION should agree"
    );
}
...
fn test_log_softmax_exp_sums_to_one() {
    ...
    let arr = get_array(
        &ex,
        "t_lsm1",
        "QUERY t_lsm1 COMPUTE res = LOG_SOFTMAX(arr) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 2);
    let sum_exp: f64 = arr.iter().map(|v| val_to_f64(v).exp()).sum();
    assert!(
        (sum_exp - 1.0).abs() < 1e-9,
        "exp(log_softmax) should sum to 1.0, got {}",
        sum_exp
    );
}
...
fn test_lse_alias() {
    ...
    let a = get_array(
        &ex,
        "t_lsm2",
        "QUERY t_lsm2 COMPUTE res = LOG_SOFTMAX(arr) SELECT res;",
        "res",
    );
    let b = get_array(
        &ex,
        "t_lsm2",
        "QUERY t_lsm2 COMPUTE res = LSE(arr) SELECT res;",
        "res",
    );
    assert_eq!(a.len(), b.len());
    for (av, bv) in a.iter().zip(b.iter()) {
        assert!(
            (val_to_f64(av) - val_to_f64(bv)).abs() < 1e-10,
            "LOG_SOFTMAX and LSE should agree"
        );
    }
}
...
fn test_mse_loss_known_value() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_mse1",
        serde_json::json!({"pred": [2.0, 3.0], "tru": [1.0, 3.0]}),
    );
    let f = get_float(
        &ex,
        "t_mse1",
        "QUERY t_mse1 COMPUTE res = MSE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (f - 0.5).abs() < 1e-9,
        "MSE_LOSS([2,3],[1,3]) should be 0.5, got {}",
        f
    );
}
...
fn test_mean_squared_error_alias() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_mse2",
        serde_json::json!({"pred": [0.0, 0.0], "tru": [1.0, 1.0]}),
    );
    let a = get_float(
        &ex,
        "t_mse2",
        "QUERY t_mse2 COMPUTE res = MSE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_mse2",
        "QUERY t_mse2 COMPUTE res = MEAN_SQUARED_ERROR(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "MSE_LOSS and MEAN_SQUARED_ERROR should agree"
    );
}
...
fn test_mae_loss_known_value() {
    ...
    let f = get_float(
        &ex,
        "t_mae1",
        "QUERY t_mae1 COMPUTE res = MAE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (f - 0.5).abs() < 1e-9,
        "MAE_LOSS([2,3],[1,3]) should be 0.5, got {}",
        f
    );
}
...
fn test_mean_absolute_error_alias() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_mae2",
        serde_json::json!({"pred": [1.0, 3.0], "tru": [0.0, 0.0]}),
    );
    let a = get_float(
        &ex,
        "t_mae2",
        "QUERY t_mae2 COMPUTE res = MAE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_mae2",
        "QUERY t_mae2 COMPUTE res = MEAN_ABSOLUTE_ERROR(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "MAE_LOSS and MEAN_ABSOLUTE_ERROR should agree"
    );
}
...
fn test_huber_loss_known_value() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_hl1",
        serde_json::json!({"yt": 1.5, "yp": 0.0, "delta": 1.0}),
    );
    let f = get_float(
        &ex,
        "t_hl1",
        "QUERY t_hl1 COMPUTE res = HUBER_LOSS(yt, yp, delta) SELECT res;",
        "res",
    );
    assert!(
        (f - 1.0).abs() < 1e-9,
        "HUBER_LOSS(1.5, 0.0, 1.0) should be 1.0, got {}",
        f
    );
}
...
fn test_smooth_l1_alias() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_hl2",
        serde_json::json!({"yt": 0.5, "yp": 0.0, "delta": 1.0}),
    );
    let a = get_float(
        &ex,
        "t_hl2",
        "QUERY t_hl2 COMPUTE res = HUBER_LOSS(yt, yp, delta) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_hl2",
        "QUERY t_hl2 COMPUTE res = SMOOTH_L1(yt, yp, delta) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "HUBER_LOSS and SMOOTH_L1 should agree"
    );
}
...
fn test_binary_cross_entropy_small_positive() {
    ...
    let f = get_float(
        &ex,
        "t_bce1",
        "QUERY t_bce1 COMPUTE res = BINARY_CROSS_ENTROPY(yt, yp) SELECT res;",
        "res",
    );
    assert!(
        f > 0.0 && f < 1.0,
        "BCE(1.0, 0.9) should be a small positive number, got {}",
        f
    );
}
...
fn test_bce_loss_alias() {
    ...
    let a = get_float(
        &ex,
        "t_bce2",
        "QUERY t_bce2 COMPUTE res = BINARY_CROSS_ENTROPY(yt, yp) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_bce2",
        "QUERY t_bce2 COMPUTE res = BCE_LOSS(yt, yp) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "BINARY_CROSS_ENTROPY and BCE_LOSS should agree"
    );
}
...
fn test_l1_regularize_known_value() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_l1r1",
        serde_json::json!({"wts": [1.0, -2.0, 3.0], "lam": 0.1}),
    );
    let f = get_float(
        &ex,
        "t_l1r1",
        "QUERY t_l1r1 COMPUTE res = L1_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
    assert!(
        (f - 0.6).abs() < 1e-9,
        "L1_REGULARIZE([1,-2,3], 0.1) should be 0.6, got {}",
        f
    );
}
...
fn test_l1_penalty_alias() {
    ...
    let a = get_float(
        &ex,
        "t_l1r2",
        "QUERY t_l1r2 COMPUTE res = L1_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_l1r2",
        "QUERY t_l1r2 COMPUTE res = L1_PENALTY(wts, lam) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "L1_REGULARIZE and L1_PENALTY should agree"
    );
}
...
fn test_l2_regularize_known_value() {
    ...
    let f = get_float(
        &ex,
        "t_l2r1",
        "QUERY t_l2r1 COMPUTE res = L2_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
    assert!(
        (f - 2.5).abs() < 1e-9,
        "L2_REGULARIZE([1,2], 1.0) should be 2.5, got {}",
        f
    );
}
...
fn test_l2_penalty_alias() {
    ...
    let a = get_float(
        &ex,
        "t_l2r2",
        "QUERY t_l2r2 COMPUTE res = L2_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_l2r2",
        "QUERY t_l2r2 COMPUTE res = L2_PENALTY(wts, lam) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "L2_REGULARIZE and L2_PENALTY should agree"
    );
}
...
fn test_clip_gradients_clips_correctly() {
    ...
    let arr = get_array(
        &ex,
        "t_cg1",
        "QUERY t_cg1 COMPUTE res = CLIP_GRADIENTS(grads, mn) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 2);
    ...
    assert!(
        (g0 - 0.6).abs() < 1e-9,
        "clipped gradient[0] should be 0.6, got {}",
        g0
    );
    assert!(
        (g1 - 0.8).abs() < 1e-9,
        "clipped gradient[1] should be 0.8, got {}",
        g1
    );
}
...
fn test_gradient_clip_alias_noop() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_cg2",
        serde_json::json!({"grads": [0.1, 0.2], "mn": 10.0}),
    );
    let arr = get_array(
        &ex,
        "t_cg2",
        "QUERY t_cg2 COMPUTE res = GRADIENT_CLIP(grads, mn) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 2);
    ...
    assert!(
        (g0 - 0.1).abs() < 1e-9,
        "gradient[0] should be unchanged at 0.1, got {}",
        g0
    );
    assert!(
        (g1 - 0.2).abs() < 1e-9,
        "gradient[1] should be unchanged at 0.2, got {}",
        g1
    );
}
...
fn test_accuracy_score_known_value() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_acc1",
        serde_json::json!({"pred": [1.0, 0.0, 1.0], "tru": [1.0, 0.0, 0.0]}),
    );
    let f = get_float(
        &ex,
        "t_acc1",
        "QUERY t_acc1 COMPUTE res = ACCURACY_SCORE(pred, tru) SELECT res;",
        "res",
    );
    let expected = 2.0 / 3.0;
    assert!(
        (f - expected).abs() < 1e-9,
        "ACCURACY_SCORE should be 2/3={}, got {}",
        expected,
        f
    );
}
...
fn test_classification_accuracy_alias() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_acc2",
        serde_json::json!({"pred": [1.0, 1.0, 0.0], "tru": [1.0, 0.0, 0.0]}),
    );
    let a = get_float(
        &ex,
        "t_acc2",
        "QUERY t_acc2 COMPUTE res = ACCURACY_SCORE(pred, tru) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_acc2",
        "QUERY t_acc2 COMPUTE res = CLASSIFICATION_ACCURACY(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (a - b).abs() < 1e-12,
        "ACCURACY_SCORE and CLASSIFICATION_ACCURACY should agree"
    );
}
...
fn test_f1_score_known_value() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_f1_1",
        serde_json::json!({"pred": [1.0, 1.0, 0.0], "tru": [1.0, 0.0, 1.0]}),
    );
    let f = get_float(
        &ex,
        "t_f1_1",
        "QUERY t_f1_1 COMPUTE res = F1_SCORE(pred, tru) SELECT res;",
        "res",
    );
...
fn test_f1_binary_alias() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_f1_2",
        serde_json::json!({"pred": [1.0, 0.0], "tru": [1.0, 0.0]}),
    );
    let a = get_float(
        &ex,
        "t_f1_2",
        "QUERY t_f1_2 COMPUTE res = F1_SCORE(pred, tru) SELECT res;",
        "res",
    );
    let b = get_float(
        &ex,
        "t_f1_2",
        "QUERY t_f1_2 COMPUTE res = F1_BINARY(pred, tru) SELECT res;",
        "res",
    );
...
fn test_f1_score_perfect_prediction() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_f1_3",
        serde_json::json!({"pred": [1.0, 1.0, 0.0, 0.0], "tru": [1.0, 1.0, 0.0, 0.0]}),
    );
    let f = get_float(
        &ex,
        "t_f1_3",
        "QUERY t_f1_3 COMPUTE res = F1_SCORE(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (f - 1.0).abs() < 1e-9,
        "F1_SCORE with perfect predictions should be 1.0, got {}",
        f
    );
}
...
fn test_f1_score_all_wrong() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_f1_4",
        serde_json::json!({"pred": [1.0, 1.0], "tru": [0.0, 0.0]}),
    );
    let f = get_float(
        &ex,
        "t_f1_4",
        "QUERY t_f1_4 COMPUTE res = F1_SCORE(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        f.abs() < 1e-9,
        "F1_SCORE with all FP should be 0.0, got {}",
        f
    );
}
...
fn test_one_hot_last_class() {
    ...
    let arr = get_array(
        &ex,
        "t_ohe4",
        "QUERY t_ohe4 COMPUTE res = ONE_HOT_ENCODE(idx, nc) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 4);
...
fn test_l1_regularize_zero_weights() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_l1r3",
        serde_json::json!({"wts": [0.0, 0.0, 0.0], "lam": 5.0}),
    );
    let f = get_float(
        &ex,
        "t_l1r3",
        "QUERY t_l1r3 COMPUTE res = L1_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
...
fn test_l2_regularize_zero_weights() {
    ...
    let f = get_float(
        &ex,
        "t_l2r3",
        "QUERY t_l2r3 COMPUTE res = L2_REGULARIZE(wts, lam) SELECT res;",
        "res",
    );
...
fn test_clip_gradients_no_clip_needed() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_cg3",
        serde_json::json!({"grads": [0.3, 0.4], "mn": 10.0}),
    );
    let arr = get_array(
        &ex,
        "t_cg3",
        "QUERY t_cg3 COMPUTE res = CLIP_GRADIENTS(grads, mn) SELECT res;",
        "res",
    );
    assert_eq!(arr.len(), 2);
...
fn test_log_softmax_length_preserved() {
    ...
    let arr = get_array(
        &ex,
        "t_lsm3",
        "QUERY t_lsm3 COMPUTE res = LOG_SOFTMAX(arr) SELECT res;",
        "res",
    );
...
fn test_accuracy_score_all_correct() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_acc3",
        serde_json::json!({"pred": [1.0, 0.0, 1.0], "tru": [1.0, 0.0, 1.0]}),
    );
    let f = get_float(
        &ex,
        "t_acc3",
        "QUERY t_acc3 COMPUTE res = ACCURACY_SCORE(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        (f - 1.0).abs() < 1e-9,
...
fn test_mse_loss_zero_error() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_mse3",
        serde_json::json!({"pred": [5.0, 5.0], "tru": [5.0, 5.0]}),
    );
    let f = get_float(
        &ex,
        "t_mse3",
        "QUERY t_mse3 COMPUTE res = MSE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        f.abs() < 1e-9,
        "MSE_LOSS of identical arrays should be 0, got {}",
        f
    );
}
...
fn test_mae_loss_zero_error() {
    ...
    let (_dir, _db, ex) = make_db(
        "t_mae3",
        serde_json::json!({"pred": [3.0, 4.0], "tru": [3.0, 4.0]}),
    );
    let f = get_float(
        &ex,
        "t_mae3",
        "QUERY t_mae3 COMPUTE res = MAE_LOSS(pred, tru) SELECT res;",
        "res",
    );
    assert!(
        f.abs() < 1e-9,
        "MAE_LOSS of identical arrays should be 0, got {}",
        f
    );
}
