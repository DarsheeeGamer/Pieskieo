...
fn test_switch() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE label = SWITCH(status, "active", "Active User", "inactive", "Inactive User", "Unknown") SELECT label;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("label"),
        Some(&Value::String("Active User".to_string()))
    );
}
...
fn test_try_parse() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"valid_int": "42", "invalid_int": "abc"}),
    )
    .unwrap();
    let mut p = Parser::new(
        r#"QUERY t COMPUTE n = TRY_PARSE(valid_int, "INT") COMPUTE bad = TRY_PARSE(invalid_int, "INT") SELECT n, bad;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_zero_if_null_and_empty_if_null() {
    ...
    .unwrap();
    let mut p = Parser::new(
        "QUERY t COMPUTE z = ZERO_IF_NULL(x) COMPUTE e = EMPTY_IF_NULL(x) SELECT z, e;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
