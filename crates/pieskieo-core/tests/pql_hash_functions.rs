...
fn test_url_encode_decode() {
    ...
    let mut p = Parser::new(
        "QUERY t COMPUTE enc = URL_ENCODE(s) COMPUTE dec = URL_DECODE(enc) SELECT dec;",
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    assert_eq!(
        r.rows[0].data.get("dec"),
        Some(&Value::String("hello world & foo=bar".to_string()))
    );
}
...
fn test_html_encode_decode() {
    ...
    db.put_doc_ns(
        ...
        Some("t"),
        Uuid::new_v4(),
        serde_json::json!({"s": "<script>alert('xss')</script>"}),
    )
    .unwrap();
    ...
    match r.rows[0].data.get("enc") {
        Some(Value::String(s)) => {
            ...
            assert!(
                s.contains("&lt;"),
                "HTML encoded string should contain &lt;"
            );
        }
