...
fn test_element_info_carbon() {
    ...
    match r.rows[0].data.get("info") {
        Some(Value::Object(obj)) => {
            ...
            assert_eq!(
                obj.get("category"),
                Some(&Value::String("nonmetal".to_string()))
            );
        }
...
fn test_molecular_weight_glucose() {
    ...
    assert!(
        (mw - 180.156).abs() < 0.5,
        "C6H12O6 mw should be ~180.156, got {}",
        mw
    );
}
...
fn test_moles_to_grams_water() {
    ...
    assert!(
        (g - 36.03).abs() < 0.5,
        "2 mol H2O should be ~36.03g, got {}",
        g
    );
}
...
fn test_mol_to_g_alias() {
    ...
    assert!(
        (g - 58.443).abs() < 0.5,
        "1 mol NaCl should be ~58.443g, got {}",
        g
    );
}
...
fn test_grams_to_moles_water() {
    ...
    assert!(
        (mol - 1.0).abs() < 0.01,
        "18.015g H2O should be ~1 mol, got {}",
        mol
    );
}
...
fn test_g_to_mol_alias() {
    ...
    assert!(
        (mol - 1.0).abs() < 0.02,
        "58.443g NaCl should be ~1 mol, got {}",
        mol
    );
}
...
fn test_concentration_alias() {
    ...
    assert!(
        (c - 2.0).abs() < 1e-9,
        "0.5 mol / 0.25 L should be 2 M, got {}",
        c
    );
}
...
fn test_dilution_volume() {
    ...
    assert!(
        (v2 - 2.0).abs() < 1e-9,
        "dilution volume should be 2.0, got {}",
        v2
    );
}
...
fn test_dilute_to_alias() {
    ...
    assert!(
        (v2 - 1.0).abs() < 1e-9,
        "dilute_to result should be 1.0, got {}",
        v2
    );
}
...
fn test_limiting_reagent_a_limits() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE lr = LIMITING_REAGENT(1.0, 2.0, 3.0, 1.0) SELECT lr;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
...
fn test_charles_law_volume() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE v2 = CHARLES_LAW_VOLUME(2.0, 300.0, 600.0) SELECT v2;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let v2 = get_float(&r, "v2");
    assert!(
        (v2 - 4.0).abs() < 1e-9,
        "Charles V2 should be 4.0, got {}",
        v2
    );
}
...
fn test_combined_gas_law_solve_v2() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE res = COMBINED_GAS_LAW(1.0, 2.0, 300.0, 2.0, NULL, 300.0) SELECT res;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    match r.rows[0].data.get("res") {
        Some(Value::Object(obj)) => {
            if let Some(Value::Float(v2)) = obj.get("V2") {
                assert!(
                    (v2 - 1.0).abs() < 1e-6,
                    "COMBINED_GAS V2 should be 1.0, got {}",
                    v2
                );
            } else {
...
fn test_henderson_hasselbalch() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE ph = HENDERSON_HASSELBALCH(4.74, 1.0, 1.0) SELECT ph;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let ph = get_float(&r, "ph");
    assert!(
        (ph - 4.74).abs() < 0.001,
        "H-H at equal concs should equal pKa=4.74, got {}",
        ph
    );
}
...
fn test_henderson_hasselbalch_eq_alias() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE ph = HENDERSON_HASSELBALCH_EQ(4.74, 1.0, 10.0) SELECT ph;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let ph = get_float(&r, "ph");
    assert!(
        (ph - 5.74).abs() < 0.01,
        "H-H with 10:1 base:acid should be 5.74, got {}",
        ph
    );
}
...
fn test_ph_from_concentration() {
    ...
    assert!(
        (ph - 7.0).abs() < 0.001,
        "pH from 1e-7 should be 7.0, got {}",
        ph
    );
}
...
fn test_ph_from_conc_alias() {
    ...
    assert!(
        (ph - 2.0).abs() < 0.001,
        "pH from 0.01 should be 2.0, got {}",
        ph
    );
}
...
fn test_concentration_from_ph() {
    ...
    assert!(
        (hc - 1e-7).abs() < 1e-9,
        "[H+] from pH 7 should be 1e-7, got {}",
        hc
    );
}
...
fn test_h_from_ph_alias() {
    ...
    assert!(
        (hc - 0.01).abs() < 1e-6,
        "[H+] from pH 2 should be 0.01, got {}",
        hc
    );
}
...
fn test_buffer_capacity() {
    ...
    assert!(
        (beta - 0.576).abs() < 0.001,
        "buffer capacity at pKa should be 0.576, got {}",
        beta
    );
}
...
fn test_buffering_capacity_alias() {
    ...
    let beta = get_float(&r, "beta");
    assert!(
        (beta - 0.0576).abs() < 0.001,
        "buffer capacity should be 0.0576, got {}",
        beta
    );
}
...
fn test_activation_energy_arrhenius() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE ea = ACTIVATION_ENERGY(1.0, 10.0, 300.0, 400.0) SELECT ea;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let ea = get_float(&r, "ea");
    assert!(
        ea > 20000.0 && ea < 30000.0,
        "Ea should be ~22941 J/mol, got {}",
        ea
    );
}
...
fn test_arrhenius_ea_alias() {
    ...
    .unwrap();
    let mut p =
        Parser::new(r#"QUERY t COMPUTE ea = ARRHENIUS_EA(1.0, 2.0, 298.0, 308.0) SELECT ea;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let ea = get_float(&r, "ea");
...
fn test_rate_constant_arrhenius() {
    ...
    let mut p = Parser::new(
        r#"QUERY t COMPUTE k = RATE_CONSTANT_ARRHENIUS(1000000000000.0, 50000.0, 298.0) SELECT k;"#,
    );
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let k = get_float(&r, "k");
...
fn test_arrhenius_k_alias() {
    ...
    assert!(
        (k - 1.0).abs() < 1e-9,
        "k with Ea=0 and A=1 should be 1.0, got {}",
        k
    );
}
...
fn test_empirical_formula_simple() {
    ...
    assert!(
        !ef.is_empty(),
        "NaCl empirical formula should not be empty, got '{}'",
        ef
    );
}
...
fn test_ph_roundtrip() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE ph2 = PH_FROM_CONCENTRATION(H_FROM_PH(4.0)) SELECT ph2;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let ph2 = get_float(&r, "ph2");
    assert!(
        (ph2 - 4.0).abs() < 0.001,
        "pH roundtrip should give 4.0, got {}",
        ph2
    );
}
...
fn test_molarity_division_by_zero() {
    ...
    assert!(
        is_null(&r, "c"),
        "MOLARITY with 0 liters should return Null"
    );
}
...
fn test_boyle_division_by_zero() {
    ...
    assert!(
        is_null(&r, "v2"),
        "BOYLE_LAW_VOLUME with P2=0 should return Null"
    );
}
...
fn test_charles_law_proportional() {
    ...
    let mut p =
        Parser::new(r#"QUERY t COMPUTE v2 = CHARLES_LAW_VOLUME(3.0, 300.0, 400.0) SELECT v2;"#);
    let r = ex.execute(p.parse().unwrap()).unwrap();
    let v2 = get_float(&r, "v2");
    assert!(
        (v2 - 4.0).abs() < 1e-9,
